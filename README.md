...
**Storage is cheap because it lives on S3-compatible object storage.** Every table is ZSTD-1 Parquet under a project prefix in your bucket of choice (Tigris, AWS S3, MinIO, local FS) — Parquet is the default on-disk format and the Iceberg / Athena / Spark interchange story. Audit-log-shaped data is 12.5× smaller than Postgres heap on the same workload. Object storage runs $0.015–$0.02/GB/mo — back-of-envelope, what costs $25/mo on Postgres-class block storage is $0.30/mo on Basin. A Vortex storage format (BtrBlocks compression cascade) — **1.95× smaller and ~20× faster to scan than ZSTD Parquet** on the audit-log dataset (see [`benchmark/RESULTS_vortex_migration.md`](./benchmark/RESULTS_vortex_migration.md)) — ships as a per-table opt-in (`CREATE TABLE … WITH (basin.file_format = 'vortex')`), validated end-to-end (codec round-trip across Int64/Utf8/Float64/Bool/Timestamp/list types incl. nulls, plus a CREATE→INSERT→SELECT test asserting `.vortex` files on disk). Vortex decode is self-describing on every read path (recovers its schema from the file, symmetric with Parquet's footer). Parquet **remains the default** to preserve lakehouse read-compat; making Vortex the default is deferred behind a bounded set of Parquet-coupled correctness subsystems (constraint/PK/FK-scan enforcement, predicate pruning, RLS, stats-driven routing, compaction) — see [ADR 0015](./docs/decisions/0015-vortex-storage-format.md).
...
| **5.0a** | Vortex storage format — 1.95× smaller / ~20× faster scan vs ZSTD Parquet | **shipped as per-table opt-in** (`WITH (basin.file_format = 'vortex')`), validated end-to-end (codec round-trip + CREATE→INSERT→SELECT). Parquet stays default; Vortex-as-default deferred ([ADR 0015](./docs/decisions/0015-vortex-storage-format.md)) |
