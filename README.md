...
_Numbers below are the live 1M-row Vortex card, regenerated by `benchmark/run/run_all.sh` (Postgres-compare cards run with the HTAP fastpaths default-on + `BASIN_FAST_BULK_INSERT=1`, as disclosed on the dashboard). Full per-row table + the 10k/100k cards in [`RESULTS_localfs.md`](./benchmark/RESULTS_localfs.md)._
