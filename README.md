...
- **Native vector search** — `vector(N)` + `<->` / `<#>` / `<=>` operators, HNSW per file segment plus a native **IVFFlat** coarse quantiser (`CREATE INDEX … USING ivfflat … WITH (lists = N)`, k-means cells + `ivfflat.probes` nearest-cell scan). `CREATE INDEX … USING hnsw WITH (m = …, ef_construction = …)` accepts pgvector-style build params, and the planner routes `ORDER BY x <-> $1 LIMIT k` to the index whose opclass (`vector_l2_ops` / `vector_cosine_ops` / `vector_ip_ops`) matches the query's distance operator. Also: `vector_avg()` element-wise mean aggregate and f32-backed `halfvec(N)`. No `pg_vector`.
- **Postgres-extension equivalents** — `pg_cron` (basin-cron), `pg_net` + `http` (basin-net), `pg_trgm` (basin-trgm), `PostGIS` subset (basin-geo, incl. `ST_AsGeoJSON` / `ST_GeomFromGeoJSON` GeoJSON I/O, R-tree envelope candidates for `&&` / `ST_DWithin`, and a general 2-D geometry surface — `LINESTRING`/`POLYGON`/`MULTI*`/`GEOMETRYCOLLECTION` WKB/EWKB/WKT/GeoJSON codecs with `ST_Length`/`ST_Area`/`ST_Centroid`/`ST_GeometryN` and exact `ST_Contains`/`ST_Intersects`, carried as WKB values with `POINT` the only native column type), `TimescaleDB` continuous aggregates (basin-cv), `pgcrypto` + `uuid-ossp` UDFs.
