...
- **10 client SDKs** — `sdk/basin-js` (TypeScript, Node/Bun/Deno/CF Workers, realtime + Arrow IPC), `sdk/basin-python` (async + sync, realtime WebSocket + Arrow IPC via pyarrow), `sdk/basin-go` (Arrow IPC), `sdk/basin-java`, `sdk/basin-rust`, `sdk/basin-ruby`, `sdk/basin-dotnet`, `sdk/basin-php`, `sdk/basin-dart`, `sdk/basin-swift`. All engine-direct (pgwire + REST, no cloud intermediary required).
...
- **Client SDKs (all in `sdk/`)** — `basin-python` (async + sync, realtime + Arrow via pyarrow), `basin-go` (Arrow IPC), `basin-java`, `basin-rust`, `basin-ruby`, `basin-dotnet`, `basin-php`, `basin-dart`, `basin-swift`. All engine-direct (pgwire + REST). 10 SDKs total.
