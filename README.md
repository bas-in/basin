...
_Numbers below are the live 1M-row Vortex card, regenerated by `benchmark/run/run_all.sh` and measured locally (Postgres-compare cards run with Basin's default config — the HTAP fastpaths and fast bulk-INSERT encoding are always-on, no non-default flags, as disclosed on the dashboard). The 1M card runs on a shared local box and is timing-sensitive; per-shape numbers reduce through a robust estimator (median at n ≥ 5 samples, min-of-K below; 1M shapes sample at least 3×) applied identically to both sides. The per-scale matrix in [`RESULTS_localfs.md`](./benchmark/RESULTS_localfs.md) and the real-S3 card below are the fuller picture._
