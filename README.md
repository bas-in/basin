...
**Storage is cheap because it lives on S3-compatible object storage.** Every table is columnar under a project prefix in your bucket of choice (Tigris, AWS S3, MinIO, local FS). The **default on-disk format is Vortex** (BtrBlocks compression cascade): **~1.95× smaller than ZSTD Parquet** on the audit-log dataset and ~59× smaller than a Postgres heap (see [`benchmark/RESULTS_vortex_migration.md`](./benchmark/RESULTS_vortex_migration.md)) — object storage at $0.015–$0.02/GB/mo means what costs $25/mo on Postgres-class block storage is well under $1/mo on Basin. The Vortex default is **correctness-verified**: zero regressions versus the prior Parquet baseline, validated by a Vortex⇆Parquet differential harness that asserts byte-identical results across point / range / aggregate / GROUP BY / ORDER BY / IS NULL / compound / DELETE / UPDATE, plus the full SQL-support matrix and ORM-compat suite. **Honest perf characterization:** Vortex's decisive wins are storage cost and scan/aggregate throughput (on-par-to-better than Parquet); on point-lookup and sort+limit shapes Vortex query *latency* is currently competitive-to-slightly-behind a mature Parquet reader, and native-execution optimization is ongoing — so **Parquet stays a first-class selectable format** for latency-critical or lakehouse-interchange tables via `CREATE TABLE … WITH (basin.file_format = 'parquet')` (Iceberg / Athena / Spark read those bytes directly). See [ADR 0015](./docs/decisions/0015-vortex-storage-format.md).
...
| **5.0a** | Vortex storage format — ~1.95× smaller than ZSTD Parquet, scan/aggregate on-par-or-better | **shipped as the DEFAULT**, zero-regression vs the Parquet baseline (differential harness + full SQL-matrix + ORM-compat green). Parquet first-class via `WITH (basin.file_format='parquet')`. Latency parity work on selective/sort shapes ongoing ([ADR 0015](./docs/decisions/0015-vortex-storage-format.md)) |
