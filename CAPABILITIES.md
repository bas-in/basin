...
| PG built-in functions — bulk scalar inventory (Phase 5.11.B) | ✅ | **String:** `char_length`/`character_length` ✅ (DF aliases), `length` ✅, `octet_length` ✅, `substring`/`substr` ✅, `position` ✅ (DF alias `strpos`), `concat` ✅, `concat_ws` ✅, `lpad` ✅, `rpad` ✅, `initcap` ✅, `ltrim`/`rtrim`/`btrim` ✅ (gap-fill UDFs), `replace` ✅, `regexp_replace` ✅, `regexp_match` ✅, `regexp_split_to_array` ✅, `split_part` ✅, `repeat` ✅, `reverse` ✅, `ascii` ✅, `chr` ✅, `md5` ✅, `encode`/`decode` ✅, `format` ✅, `quote_ident`/`quote_literal` ✅, `translate` ✅. **Math:** `abs` ✅, `ceil` ✅, `ceiling` ✅ (alias), `floor` ✅, `round` ✅, `trunc` ✅, `mod`/`%` ✅, `power`/`pow` ✅, `sqrt` ✅, `exp` ✅, `ln` ✅, `log` ✅, `pi` ✅, `random` ✅, `sign` ✅ (alias for `signum`), `width_bucket` ✅, `degrees` ✅, `radians` ✅, `sin`/`cos`/`tan`/`asin`/`acos`/`atan`/`atan2` ✅. **Datetime:** `now` ✅, `current_timestamp` ✅, `current_date` ✅, `current_time` ✅, `localtime` ✅ (stub), `localtimestamp` ✅ (stub), `clock_timestamp` ✅ (stub=now), `transaction_timestamp` ✅ (stub=now), `statement_timestamp` ✅ (stub=now), `make_date` ✅, `make_time` ✅, `make_timestamp` ✅, `make_interval` 🛠 (stub, returns "0 seconds"), `to_timestamp` ✅, `to_date` ✅. **Null/Cond:** `coalesce`/`nullif`/`greatest`/`least` ✅. **Aggregate:** `count`/`sum`/`avg`/`min`/`max` ✅, `array_agg` ✅, `string_agg` ✅, `bool_and`/`bool_or` ✅, `every` ✅ (rewrites to `bool_and`), `bit_and`/`bit_or` ✅. **Type/format:** `to_number` ✅ (numeric-text parser), `to_char` ✅, `format_type` ✅ (pg_catalog stub), `quote_ident`/`quote_literal` ✅. Test coverage: `tests/integration/tests/pg_scalar_fn_inventory.rs` (Phase 5.11.B). |
