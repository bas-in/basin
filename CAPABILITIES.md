...
| `int[][]` / multi-dimensional arrays | ✅ | P10 — Arrow `List(List(T))` recursive bridge (`convert.rs`). `'{{1,2},{3,4}}'::int[][]` round-trips through the engine; `array_dims` returns `'[1:outer][1:inner]'`. Schema translation (`ws_to_df` / `df_to_ws`) and batch conversion both handle arbitrary nesting depth recursively. |
