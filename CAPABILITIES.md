...
| JSONB scalar UDFs (21 functions from sibling commit) | ✅ | `jsonb_typeof`, `jsonb_pretty`, `jsonb_array_length`, `jsonb_strip_nulls`, `jsonb_set`, `jsonb_insert`, `jsonb_path_query` (first-match scalar), `jsonb_path_exists`, `jsonb_path_match`, `jsonb_object_keys` (scalar/stub), `jsonb_each` (scalar/stub), `jsonb_each_text` (scalar/stub), `jsonb_array_elements` (scalar/stub), `jsonb_array_elements_text` (scalar/stub), `jsonb_build_object`, `jsonb_build_array`, `to_jsonb`, `row_to_json`, `array_to_json`, `jsonb_agg` (stub), `jsonb_object_agg` (stub). All accept `LargeBinary` or `Utf8` inputs. |
| JSON path query extras (PG 12+) | ✅ | `jsonb_path_query_first(jsonb, jsonpath)` — first match; `jsonb_path_query_array(jsonb, jsonpath)` — all matches as JSONB array. Minimal jsonpath parser supports `$.field`, `$.field[0]`, `$.field[*]`, `$.**` (recursive descent), filter predicates `? (@.x > 1)` stripped and ignored (documented limitation). `@?` operator rewritten to `jsonb_path_exists` call. |
| JSON conversion functions | ✅ | `json_typeof(json)`, `json_strip_nulls(json)`, `to_json(any)` (returns Utf8); `json_each(json)`, `json_each_text(json)`, `json_object_keys(json)`, `json_array_elements(json)`, `json_array_elements_text(json)` — scalar stubs returning first element / comma-joined keys (true SRF deferred to v0.2); `json_to_record` / `jsonb_to_record` / `json_to_recordset` / `jsonb_to_recordset` — stub-errors directing to v0.2. |
| JSONB operator text rewriters | ✅ | `->` → `json_get(j, key)`, `->>` → `json_get_text(j, key)`, `#>` → `json_path_extract(j, path)`, `#>>` → `json_path_extract_text(j, path)`, `@>` (JSON only, heuristic) → `jsonb_contains`, `<@` → `jsonb_contained_by`, `?` → `jsonb_has_key`, `?&` → `jsonb_has_all_keys`, `?|` → `jsonb_has_any_key`, `\|\|` (JSON RHS) → `jsonb_concat`, `@?` → `jsonb_path_exists`. All UDFs registered in `jsonb_udf.rs`; textual rewriter in `udf::rewrite_json_operators` wired before sqlparser in `executor.rs`. Range-type `@>` unaffected (RHS-heuristic guard). |
