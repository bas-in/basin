...
| `ALTER TABLE … DROP COLUMN [IF EXISTS]` | ✅ | Phase 5.12. Schema-only metadata change — existing parquet files keep their original schema; the reader projects on the new shape, so the dropped column simply stops appearing in result sets. Refuses to drop a column that participates in PK / UNIQUE / FK; CHECK predicates that textually reference the column are dropped alongside (PG-RESTRICT-equivalent). Verified by `tests/integration/tests/alter_table_std.rs`. |
| `ALTER TABLE … RENAME COLUMN` | ✅ | Phase 5.12. Metadata rename — existing parquet files keep their original column name and the reader maps physical-to-logical at scan time. Rejects when the target name is already used. Verified by `tests/integration/tests/alter_table_std.rs`. |
| `ALTER TABLE … RENAME TO` | ✅ | Phase 5.12. New catalog key aliases the existing TableState; the old key stays as a synonym until DROP so the engine's session-level refresh finds the table under either name (v0.2 tombstones the old key via an alias map). Verified by `tests/integration/tests/alter_table_std.rs`. |
| `ALTER TABLE … ALTER COLUMN … [SET DATA] TYPE` | ✅ | Phase 5.12. Arrow `DataType` swap on the catalog schema; existing parquet rows stay on disk in the old type and the reader best-effort coerces at scan time (Arrow's cast helper covers the common int→bigint widening). `SET/DROP DEFAULT`, `SET/DROP NOT NULL` accepted as metadata-only no-ops so multi-step PG migrations aren't blocked. `ADD GENERATED` rejected (declare on CREATE). Verified by `tests/integration/tests/alter_table_std.rs`. |
| `ALTER TABLE … ADD CONSTRAINT … CHECK (…)` | ✅ | Phase 5.12. Appends a `CheckConstraint` row to the catalog; validated by the engine on subsequent writes only (PG `NOT VALID` semantics, just without the explicit syntax). Duplicate-name rejected. UNIQUE / PK / FK additions remain deferred (require backfill validation). Verified by `tests/integration/tests/alter_table_std.rs`. |
| `ALTER TABLE … DROP CONSTRAINT [IF EXISTS]` | ✅ | Phase 5.12. Removes a named CHECK / UNIQUE / FK / PK (via `<table>_pkey` convention) from the catalog. Verified by `tests/integration/tests/alter_table_std.rs`. |
| `ALTER TABLE … VALIDATE CONSTRAINT` | ✅ | Phase 5.12. Textual matcher (sqlparser 0.52 doesn't model the PG form). Basin enforces every constraint on every write, so VALIDATE is a no-op accept on a known constraint and SQLSTATE 42704 on an unknown one. Verified by `tests/integration/tests/alter_table_std.rs`. |
| `ALTER TABLE … ATTACH PARTITION` / `DETACH PARTITION` | ✅ | Phase 5.12. Declarative partitions are computed from the PARTITION BY column at write time, so the attach / detach statements carry no state change for Basin — accepted syntactically so PG-shaped migration tooling doesn't get rejected at parse time. Verified by `tests/integration/tests/alter_table_std.rs`. |
| `ALTER TABLE … SET cold_after = '7d'` (quoted duration) | ✅ | Phase 5.12. Extension of the existing `SET cold_after = N` form: `'7d'` / `'12h'` / `'30m'` / `'45s'` / `'2w'` parse to seconds. Bare integer continues to mean raw seconds. Verified by `tests/integration/tests/alter_table_std.rs`. |
| `CREATE TABLE … REFERENCES <t>` (implicit-PK FK) | ✅ | Phase 5.12. PG-shape `REFERENCES <t>` without a column list means "the PK of <t>"; v0.1 accepts the syntax and drops the FK from the catalog payload (degraded to declarative — write-time enforcement returns when async PK resolution lands in v0.2). Verified by `tests/integration/tests/alter_table_std.rs`. |
| `CREATE TABLE … REFERENCES … ON DELETE {SET NULL,SET DEFAULT,RESTRICT}` | ✅ | Phase 5.12. v0.1's `RefAction` enum carries only `NoAction`/`Cascade`; the remaining PG actions degrade to `NoAction` so migration DDL doesn't get rejected at parse time. CASCADE continues to enforce. Verified by `tests/integration/tests/alter_table_std.rs`. |
| `CREATE UNLOGGED TABLE` | ✅ | Phase 5.12. `UNLOGGED` keyword stripped pre-parse (Basin's storage doesn't differentiate logged/unlogged). Verified by `tests/integration/tests/alter_table_std.rs`. |
| `CREATE TABLE … INHERITS (parent)` | ✅ | Phase 5.12. `INHERITS (…)` clause stripped pre-parse; parent's columns are not copied today (deferred to v0.2 once the engine has async access to peer schemas at CREATE time). Empty-column-list CREATEs synthesise a `_basin_placeholder BOOLEAN` field so the Arrow schema stays non-empty. Verified by `tests/integration/tests/alter_table_std.rs`. |
| `CREATE TABLE … (LIKE source INCLUDING ALL)` | ✅ | Phase 5.12. `(LIKE <src> [INCLUDING …] [EXCLUDING …])` stripped pre-parse out of the column list; source's columns are not copied today (same v0.2 deferral as INHERITS). Verified by `tests/integration/tests/alter_table_std.rs`. |
| `UNIQUE / PRIMARY KEY … INCLUDE (col)` | ✅ | Phase 5.12. PG covering-index `INCLUDE` clause stripped pre-parse; Basin doesn't materialise an INCLUDE-shaped index in v0.1 (secondary indexes are Phase 5.7 B1). Verified by `tests/integration/tests/alter_table_std.rs`. |
| `FOREIGN KEY … REFERENCES … MATCH FULL` | ✅ | Phase 5.12. `MATCH FULL` / `MATCH PARTIAL` / `MATCH SIMPLE` stripped pre-parse; Basin treats every FK as MATCH SIMPLE. Verified by `tests/integration/tests/alter_table_std.rs`. |
| `PARTITION BY LIST (col)` / `PARTITION BY HASH (col)` | ✅ | Phase 5.12. Accepted syntactically and recorded as `Unpartitioned`; LIST / HASH semantics are an unenforced declaration that survives the DDL but doesn't change reader behaviour. RANGE remains the enforced strategy. Verified by `tests/integration/tests/alter_table_std.rs`. |
