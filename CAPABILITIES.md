...
| `CAST(x AS type)` — SQL-standard explicit cast | ✅ | Handled by DataFusion's `PostgreSqlDialect` planner. Supports: `BIGINT`/`INTEGER`/`SMALLINT`, `FLOAT4`/`FLOAT8`/`DOUBLE PRECISION`, `TEXT`/`VARCHAR`, `DATE`, `TIMESTAMP`, `NUMERIC(p,s)`, `BOOLEAN`. Truncating (float→int) and widening (int→float) casts both work. Exercised by `tests/integration/tests/pg_type_casts.rs`. |
| `x::type` — PG shorthand cast operator | ✅ | `PostgreSqlDialect` parses `::type` suffix and lowers it to the same `CAST` AST node. Identical semantics to `CAST(x AS type)`. `42::BIGINT`, `1::FLOAT8`, `'hello'::TEXT`, `'2024-01-01'::DATE` all work. Exercised by `tests/integration/tests/pg_type_casts.rs`. |
| `'<literal>'::int` / `'<literal>'::float8` — string-to-numeric cast | ✅ | String literals cast to integer or float via DataFusion's `CAST(Utf8 → Int*/Float*)` coercion. `'42'::BIGINT` → 42, `'3.14'::FLOAT8` → 3.14. Exercised by `tests/integration/tests/pg_type_casts.rs`. |
| `'<literal>'::numeric(p,s)` — string-to-NUMERIC cast | ✅ | String literal cast to `NUMERIC(p,s)`; uses the same `coerce_decimal128` path that INSERT uses. Works in `WHERE` predicates and `SELECT` expressions. Exercised by `tests/integration/tests/pg_type_casts.rs`. |
| `1::text` / `CAST(n AS TEXT)` — integer/float-to-text cast | ✅ | Numeric and boolean values cast to `TEXT` via DataFusion's `CAST(* → Utf8)` coercion. Common in PG for log columns, concat expressions, and `||` string joins. Exercised by `tests/integration/tests/pg_type_casts.rs`. |
| `'2024-01-15'::date` — string-to-date cast | ✅ | ISO-8601 date string cast to `DATE` (Arrow `Date32`). Parsed by DataFusion's `CAST(Utf8 → Date32)`. Produces the correct days-since-epoch value. Exercised by `tests/integration/tests/pg_type_casts.rs`. |
| `int4 → int8` numeric width promotion | ✅ | `INTEGER` column values cast to `BIGINT` via `CAST(small AS BIGINT)`. DataFusion coerces `Int32 → Int64` transparently in arithmetic expressions and explicit casts. Exercised by `tests/integration/tests/pg_type_casts.rs`. |
| `to_number(text, format)` — PG format-driven numeric parse | ✅ | `to_number('1,234.56', '9,999.99')` → 1234.56 (Float64). Supports PG format-picture characters: `9`/`0` (digit), `,`/`G` (thousands sep), `.`/`D` (decimal), `S`/`MI` (sign). Registered as a `ScalarUDF` in `register_pg_compat_udfs`; commonly used in reporting / import pipelines. Exercised by `tests/integration/tests/pg_type_casts.rs`. |
| `to_date(text, format)` — PG format-driven date parse | ✅ | `to_date('2024-01-15', 'YYYY-MM-DD')` → `Date32`. Translates PG format directives (`YYYY`, `MM`, `DD`, etc.) via the same `pg_format_to_chrono` mapping used by `to_char`/`to_timestamp`. Registered as a `ScalarUDF` in `register_pg_compat_udfs`. Exercised by `tests/integration/tests/pg_type_casts.rs`. |
| `to_char(date, format)` — date-to-string formatting | ✅ | `to_char(dt, 'YYYY-MM-DD')` renders a `Date32` column as a formatted text string. The existing `to_char` UDF (Phase 5.11.A) accepts `Date32` inputs alongside `Timestamp*` inputs. Exercised by `tests/integration/tests/pg_type_casts.rs`. |
| `CAST(float AS BIGINT)` — truncating float-to-integer cast | ✅ | Truncates toward zero (PG semantics): `CAST(3.9 AS BIGINT)` → 3, `CAST(-2.7 AS BIGINT)` → -2. DataFusion's `CastExpr` applies C-style truncation for float→integer. Exercised by `tests/integration/tests/pg_type_casts.rs`. |
