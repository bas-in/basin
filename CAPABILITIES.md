...
| **Basic aggregate functions** | ✅ | Phase 5.11.AGG. `count(*)`, `count(DISTINCT col)`, `sum`, `avg`, `min`, `max` — all native DataFusion. Verified in `tests/integration/tests/pg_aggregates.rs`. |
| `bool_and(col)` / `bool_or(col)` | ✅ | Native DataFusion aggregates. PG synonym `every(col)` rewritten to `bool_and(col)` by the `rewrite_pg_agg_aliases` pre-pass in `basin-engine::udf`. Verified in `pg_aggregates.rs::basic_bool_aggregates`. |
| `bit_and(col)` / `bit_or(col)` | ✅ | Native DataFusion aggregates on BIGINT columns. Verified in `pg_aggregates.rs::basic_bit_aggregates`. |
| `array_agg(col)` | 🛠 | DataFusion executes `array_agg` without error but the output `List` Arrow type is not yet bridged through basin's df↔ws schema converter — queries succeed inside the engine but fail over the pgwire path. `ORDER BY` inside the aggregate is documented in DataFusion but also blocked by the List bridge gap. Fix: add `List` support to `basin-engine::convert`. |
| `string_agg(col, delim)` | 🛠 | DataFusion executes `string_agg` but returns `LargeUtf8` which basin's schema converter does not yet bridge to `Utf8`. The aggregate is available at the engine level; the output type converter shim is the remaining gap. |
| **Statistical aggregates** | ✅ | Phase 5.11.AGG. All DataFusion native: `stddev(col)` (= `stddev_samp`), `stddev_pop(col)`, `var_samp(col)` (= `var`), `var_pop(col)`, `corr(y, x)`, `covar_samp(y, x)`, `covar_pop(y, x)`, `regr_slope(y, x)`, `regr_intercept(y, x)`, `regr_r2(y, x)`, `regr_avgx(y, x)`, `regr_avgy(y, x)`. PG name `variance(col)` rewritten to `var(col)` by `rewrite_pg_agg_aliases`. Verified by `pg_aggregates.rs` test suite. |
| `variance(col)` PG alias | ✅ | Pre-plan SQL rewrite in `basin-engine::udf::rewrite_pg_agg_aliases` maps `variance(` → `var(` (DataFusion's primary name) before the planner sees the query. |
| `approx_percentile_cont(col, frac)` | ✅ | Native DataFusion approximate percentile (T-Digest). PG's `percentile_cont(frac) WITHIN GROUP (ORDER BY col)` ordered-set syntax is not yet parsed by sqlparser — use the DataFusion form directly. |
| `percentile_cont` / `percentile_disc` WITHIN GROUP | 🛠 | PG ordered-set aggregate syntax (`WITHIN GROUP (ORDER BY col)`) is not supported by sqlparser 0.52. DataFusion's `approx_percentile_cont(col, frac)` provides equivalent approximate semantics with different call syntax. |
| `mode() WITHIN GROUP (ORDER BY col)` | 🛠 | Ordered-set aggregate syntax not supported by sqlparser 0.52. No DataFusion equivalent. |
| `json_agg(col)` | ✅ | Phase 5.11.AGG. Custom UDAF in `basin-engine::pg_agg_udf` accumulates column values into a JSON array string. Handles `NULL` (emits `null` in array). Registered on every session. Verified by `pg_aggregates.rs::json_agg_basic`. |
| `jsonb_agg(col)` | ✅ | Identical implementation to `json_agg` — registered as a separate UDAF alias. Output type is `TEXT` (JSONB/JSON are both stored as UTF-8 text in basin). Verified by `pg_aggregates.rs::jsonb_agg_basic`. |
| `json_object_agg(key, value)` | ✅ | Phase 5.11.AGG. Custom UDAF in `basin-engine::pg_agg_udf` accumulates `(key, value)` pairs into a JSON object string. Key must be non-NULL. NULL values become `null` in the object. Verified by `pg_aggregates.rs::json_object_agg_basic`. |
| `jsonb_object_agg(key, value)` | ✅ | Identical implementation to `json_object_agg`. Verified by `pg_aggregates.rs::jsonb_object_agg_basic`. |
| `json_build_object` / `json_build_array` | 🛠 | Scalar constructors (not aggregates). Not yet registered as UDFs; `jsonb_build_object` / `jsonb_build_array` variants also absent. Planned alongside `to_jsonb` / `row_to_json` / `array_to_json`. |
