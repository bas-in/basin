...
| `DECLARE <name> [SCROLL] CURSOR FOR <query>` | ✅ | Per-session cursor registry (`CursorRegistry` in `cursor.rs`). Result is fully materialised at DECLARE time into a `Vec<RecordBatch>`. `SCROLL` / `NO SCROLL` keyword accepted (ignored in v0.1 — all materialised cursors support backward motion). `WITH HOLD` silently ignored (cursors are session-scoped). Duplicate-name raises PG-style error. |
| `FETCH [direction] FROM <cursor>` | ✅ | Full PG direction surface: `NEXT`, `PRIOR`, `FIRST`, `LAST`, `ALL`, `FORWARD [n\|ALL]`, `BACKWARD [n\|ALL]`, `ABSOLUTE n`, `RELATIVE n`, `COUNT n`. Slices Arrow batches from the materialised result and advances the logical row pointer. Returns `ExecResult::Rows` with the cursor's schema. |
| `MOVE [direction] FROM <cursor>` | ✅ | Advances the cursor position without returning rows (`ExecResult::Empty { tag: "MOVE" }`). Uses the same direction grammar as `FETCH`. Pre-screened textually before sqlparser sees the SQL (sqlparser 0.52 has no `Statement::Move` AST node); the synthetic-FETCH trick converts the direction tokens via the existing `FetchDirection` parser. |
| `CLOSE <cursor>` | ✅ | Removes the cursor from the per-session registry and drops its materialised batches. PG-style error if the cursor does not exist. `CLOSE ALL` returns an error in v0.1 (individual close only). |
