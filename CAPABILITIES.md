...
| `pg_stat_*` monitoring views (10 new) | ✅ | `pg_stat_database` (single tenant row with stub counters), `pg_stat_bgwriter` (all-zero row — no bgwriter), `pg_stat_replication` / `pg_stat_wal_receiver` / `pg_stat_subscription` (empty — no replication), `pg_stat_archiver` (empty), `pg_stat_user_functions` (empty), `pg_stat_progress_vacuum` / `pg_stat_progress_create_index` / `pg_stat_progress_analyze` (empty). Registered in `pg_catalog` schema. Tested in `tests/integration/tests/catalog_extras2.rs`. |
| `information_schema` privilege + role views | ✅ | `check_constraints` (empty — no SQL CHECK enforcement yet), `triggers` (empty), `usage_privileges` (empty), `table_privileges` (all-allow rows for calling tenant), `column_privileges` / `role_column_grants` / `role_routine_grants` (empty — no per-column/routine grants), `applicable_roles` + `enabled_roles` (single calling-tenant row). |
| `information_schema` FDW / foreign-table views (8) | ✅ | `foreign_data_wrappers`, `foreign_data_wrapper_options`, `foreign_servers`, `foreign_server_options`, `foreign_tables`, `foreign_table_options`, `user_mappings`, `user_mapping_options` — all empty stubs (no FDW support in basin v0.1). Registered so ORM and admin-tool introspection queries do not fail with table-not-found. |
