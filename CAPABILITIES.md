...
| `IS TRUE` / `IS NOT TRUE` / `IS FALSE` / `IS NOT FALSE` | ✅ | DataFusion-native via sqlparser `Expr::IsTrue` / `Expr::IsNotTrue` / `Expr::IsFalse` / `Expr::IsNotFalse`. Three-valued-logic semantics: `NULL IS TRUE` → false, `NULL IS FALSE` → false. Works in SELECT projections and WHERE predicates over nullable BOOLEAN columns. Tested by `tests/integration/tests/is_operators.rs`. |
| `IS UNKNOWN` / `IS NOT UNKNOWN` | ✅ | DataFusion-native; sqlparser `Expr::IsUnknown` / `Expr::IsNotUnknown`. Semantically equivalent to `IS NULL` / `IS NOT NULL` on boolean domains per SQL:2011 §6.28. Tested by `tests/integration/tests/is_operators.rs`. |
| `IS NULL` / `IS NOT NULL` (scalar + row predicate) | ✅ | DataFusion-native. Works on scalar expressions and nullable table columns; pushes down into Parquet predicate evaluation. Tested by `tests/integration/tests/is_operators.rs`. |
| `IS DISTINCT FROM` / `IS NOT DISTINCT FROM` | ✅ | DataFusion-native (`Expr::IsDistinctFrom` / `Expr::IsNotDistinctFrom`). NULL-safe equality: `NULL IS NOT DISTINCT FROM NULL` → true. Tested by `tests/integration/tests/is_operators.rs`. |
| `COALESCE(a, b, c, ...)` (3+ arguments) | ✅ | DataFusion built-in; accepts any number of args ≥ 2. Returns the first non-NULL argument. Works in SELECT, WHERE, and UPDATE SET. Tested by `tests/integration/tests/is_operators.rs`. |
| `NULLIF(a, b)` | ✅ | DataFusion built-in. Returns NULL when both args are equal; otherwise returns first arg. Tested by `tests/integration/tests/is_operators.rs`. |
| `GREATEST(a, b, ...)` / `LEAST(a, b, ...)` | ✅ | DataFusion built-in; NULL inputs ignored per PG semantics. Works on numeric and text types with 2+ arguments. Tested by `tests/integration/tests/is_operators.rs`. |
| `CASE WHEN … THEN … ELSE … END` (searched form) | ✅ | DataFusion-native. Multi-branch; ELSE-less CASE returns NULL when no branch matches. Works in SELECT projections and WHERE. Tested by `tests/integration/tests/is_operators.rs`. |
| `CASE expr WHEN val THEN … ELSE … END` (simple form) | ✅ | DataFusion-native. Equality-match on the leading expression; falls to ELSE (or NULL if absent) when no value matches. Tested by `tests/integration/tests/is_operators.rs`. |
