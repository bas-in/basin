...
| `LISTEN` / `NOTIFY` | 🚫 | replaced by basin-realtime SSE + WebSocket (see Auth and REST API section) |
...
| `CREATE FUNCTION … LANGUAGE wasm` (WASM UDFs) | ✅ | Phase 5.11.J (`fa65bcd`). Wasmtime-backed execution; `.wasm` module registered in catalog; call-site dispatch via `ScalarUDF` bridge. Same `CREATE FUNCTION` / `DROP FUNCTION` DDL surface as `LANGUAGE sql`. |
...
| Per-project secondary indexes (B-tree) | ✅ | Phase 5.7 B1 (`33a8162`). `CREATE INDEX <name> ON <table> (<col>)` — fast-select probe path; `pg_index` view populates with real rows. Biggest point-query win for true random ids. |
...
| Diesel | untested (needs diesel CLI + libpq in CI) | 4-migration fixture (including a JSONB column) at `tests/integration/fixtures/migration-tool-scaffold/diesel/migrations/`. Test (`migration_tool_diesel.rs`) covers `diesel migration run`, `revert`, JSONB round-trip via SQL (simulating Diesel's `Jsonb` type), and tracking-table assertion on `__diesel_schema_migrations` (one row per applied migration). Skips cleanly when `diesel` binary is absent. Known risks: `__diesel_schema_migrations` CREATE TABLE IF NOT EXISTS; `SHOW`/`SET search_path`; `pg_advisory_lock` (some diesel-cli versions); JSONB DDL + wire encoding. Install: `cargo install diesel_cli --no-default-features --features postgres` (requires libpq-dev / postgresql first). |
| sqlx | untested (needs sqlx-cli in CI) | 5-migration up-only fixture at `tests/integration/fixtures/migration-tool-scaffold/sqlx/`. Test (`migration_tool_sqlx.rs`) covers `sqlx migrate run`, tracking-table assertion on `_sqlx_migrations` (version BIGINT, description TEXT, success BOOLEAN, checksum BYTEA, execution_time BIGINT), full CRUD battery. Skips cleanly when `sqlx` binary is absent. Note: compile-time `query!` macro checking is NOT exercised (requires a live DB at build time or a pre-generated `sqlx-data.json`; out of scope for this test). Known risks: `BYTEA` in `_sqlx_migrations.checksum`; `TIMESTAMPTZ` in `_sqlx_migrations.installed_on`; `pg_advisory_lock` (some sqlx-cli versions); `migrate revert` exits non-zero for up-only migrations (expected). Install: `cargo install sqlx-cli --no-default-features --features native-tls,postgres`. |
| Prisma | untested (needs Node ≥ 18 + prisma CLI in CI) | Schema fixture at `tests/integration/fixtures/migration-tool-scaffold/prisma/schema.prisma` (4 models: User, Team, Post, Comment with relations). Test (`migration_tool_prisma.rs`) uses `prisma db push` (NOT `migrate dev`) because Basin does not support `CREATE DATABASE` for Prisma's shadow-database mechanism. Covers schema push, schema introspection assertion, Prisma Client-style CRUD battery via `tokio_postgres`. Skips cleanly when `node` or `prisma` is absent. Primary known risks: pg_catalog introspection gaps (`pg_class`, `pg_attribute`, `pg_constraint`, `pg_index`, `pg_enum`, `pg_type` — Prisma reads all of these heavily); `_prisma_migrations` table is NOT created by `db push` (only by `migrate deploy`); `SERIAL` / `autoincrement()` default reflection; FK constraint rows in `pg_constraint`; extended-protocol parameter type inference via `pg_type` OIDs. Install: `npm install -g prisma` (Node ≥ 18 required). |
