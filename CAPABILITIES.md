...
| `INTERVAL`, `MONEY`, `XML` | 🚫 | |
| geometric (`LINESTRING`/`POLYGON`/`MULTI*`/`GEOMETRYCOLLECTION`) | 🛠 | Value-level support via `basin-geo` (WKB/EWKB/WKT/GeoJSON codecs + measures + exact predicates); carried as `BYTEA`/WKB, **not** a native column DDL type. Only `POINT` has a native column type + R-tree index. |
...
| `PostGIS` (subset) | **`basin-geo`** | ✅ | `Point`, `Box2d`, `ST_MakePoint`, `ST_X`, `ST_Y`, `ST_Distance` (Haversine WGS84), `ST_DWithin`, `ST_Contains`. **GeoJSON I/O**: `ST_AsGeoJSON` / `ST_GeomFromGeoJSON` (`41639b8`), with constructor-expression INSERTs. **General 2-D geometry surface**: `LINESTRING` / `POLYGON` (+holes) / `MULTIPOINT` / `MULTILINESTRING` / `MULTIPOLYGON` / `GEOMETRYCOLLECTION` via variable-length WKB/EWKB/WKT/GeoJSON codecs — `ST_GeomFromText` / `ST_GeomFromGeoJSON` / `ST_GeomFromWKB`, `ST_AsText` / `ST_AsGeoJSON` / `ST_AsEWKB`, `ST_GeometryType`, `ST_NumPoints`, `ST_NumGeometries`, `ST_GeometryN`, `ST_PointN`, `ST_StartPoint` / `ST_EndPoint` / `ST_ExteriorRing`, `ST_Length` / `ST_Area` / `ST_Perimeter` (planar + WGS84-`geography`), `ST_Centroid`, `ST_Envelope`, and exact-planar `ST_Intersects` / `ST_Contains` / `ST_Within` (via the `geo` crate's topology traits — not bbox approximations). **R-tree index**: row-exact envelope candidates (`a250891`) — `&&` bbox-overlap and `ST_DWithin` radius queries prune to candidate row groups via an in-memory R-tree before decoding; `&&` does a single-decode residual to avoid re-decoding each surviving row's WKB (`dd26766`). Measured: `ST_DWithin` ~1.7× faster than PostGIS GIST on the R-tree compare card, and ~28× faster than an unindexed Basin scan at 1M rows (`viability_large_spatial_dwithin`). **Loss:** `&&` bbox-count and KNN `<->` LIMIT still trail PostGIS GIST by ~150× — PG's GIST is structurally faster on those two shapes. Only `POINT` has a native column DDL type; general geometries ride as `BYTEA`/WKB values, and the R-tree index covers `POINT` only. |
