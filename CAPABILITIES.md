...
| `UPDATE … SET col = <expression>` | ✅ | Full expression RHS via DataFusion: arithmetic (`SET cnt = cnt + 1`, `SET v = v * 2`), string functions (`SET w = upper(w)`, `SET name = name \|\| ' (updated)'`), conditionals (`CASE WHEN … END`), multi-arg functions (`LEAST(col, 100)`, `coalesce(col, 0)`). Literal and bind-parameter (`$1`) fast path preserved. Non-matched rows in a partial-WHERE UPDATE keep their original values. Exercised by `tests/integration/tests/dml_expressions.rs`. |
| `DELETE … RETURNING col / *` | ✅ | Returns deleted rows as a result set. Supports `RETURNING *` (all columns) and `RETURNING col` (column subset). Works on both the AllMatch (file-drop) and Mixed (per-row filter) DELETE paths. Exercised by `tests/integration/tests/dml_expressions.rs`. |
