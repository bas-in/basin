...
| FTS `@@` operator (`tsvector @@ tsquery`) | ✅ | DataFusion has no native handler for `@@`. `basin-engine::pg_ast::rewrite_tsvector_at_at` lowers every `lhs @@ rhs` to `tsvector_match_udf(lhs, rhs)` in the SQL string before sqlparser sees it. `tsvector_match_udf` is a v0.1 stub returning `true` (match-all); real selectivity deferred to `basin-fts`. |
| FTS scalar UDFs (`to_tsvector`, `to_tsquery`, `ts_rank`, etc.) | ✅ | v0.1 stubs registered via `fts_udf::register_fts_udfs`. Functions: `to_tsvector(text)`, `to_tsvector(config, text)`, `to_tsquery(text)`, `to_tsquery(config, text)`, `plainto_tsquery`, `phraseto_tsquery`, `websearch_to_tsquery` (1/2-arg each), `ts_rank(tsvector, tsquery)`, `ts_rank_cd(tsvector, tsquery)` → `Float32 0.0`, `ts_headline(text, tsquery)` / `ts_headline(config, text, tsquery)` → echo body, `setweight(tsvector, weight)` → echo tsvector, `strip(tsvector)` → echo tsvector, `tsvector_length(tsvector)` → word count (`Int32`), `numnode(tsquery)` → `1` (`Int32`), `querytree(tsquery)` → echo query, `tsvector_match(tsvector, tsquery)` → `false`, `tsvector_match_udf(tsvector, tsquery)` → `true`. ORM migration scripts and FTS-shaped queries plan and execute without error. Real tokenisation/selectivity deferred to `basin-fts`. |
...
| `TSVECTOR` | ✅ | Arrow `Utf8` + field metadata `BASIN_TYPE=TSVECTOR`; DDL `CREATE TABLE … (col TSVECTOR)` accepted; `to_tsvector`, `setweight`, `strip`, `tsvector_length` UDFs available (v0.1 stubs). pgwire type metadata set so downstream layers advertise OID 3614 rather than plain-text OID. |
| `TSQUERY` | ✅ | Arrow `Utf8` + field metadata `BASIN_TYPE=TSQUERY`; DDL `CREATE TABLE … (col TSQUERY)` accepted; `to_tsquery`, `plainto_tsquery`, `phraseto_tsquery`, `websearch_to_tsquery`, `numnode`, `querytree` UDFs available (v0.1 stubs). pgwire type metadata set for OID 3615. |
