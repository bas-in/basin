...
| `CREATE INDEX name ON tbl (col)` — basic single-column | ✅ | Accepted and stored in catalog as metadata; actual query plans still use full scan in v0.1. Full physical index wires in when Phase 5.7 B1 ships. Tested in `tests/integration/tests/index_variants.rs`. |
| `CREATE UNIQUE INDEX` | ✅ | Uniqueness flag accepted and logged as metadata; uniqueness is **not enforced** at write time in v0.1 — declare `UNIQUE (col)` at `CREATE TABLE` for enforcement. Accepted as metadata per `tests/integration/tests/index_variants.rs`. |
| `CREATE INDEX … WHERE predicate` (partial index) | ✅ | `WHERE` predicate accepted and stored as metadata string; partial-index pruning not applied in v0.1. All valid PG predicate syntax is parsed by sqlparser 0.52. Tested in `tests/integration/tests/index_variants.rs`. |
| `CREATE INDEX … ON tbl ((expr))` (expression index) | ✅ | Expression columns (e.g. `LOWER(name)`, `col1 + col2`) accepted and stored with an `expr:` prefix in catalog metadata; expression evaluation at index-scan time deferred to v0.2. Tested in `tests/integration/tests/index_variants.rs`. |
| `CREATE INDEX … USING method` (btree / hash / gin / gist) | ✅ | Any access-method name accepted; non-btree methods are remapped to basin's single native index shape and logged. Enables schema-migration scripts that specify GIN/GiST/hash indexes to run without modification. Tested in `tests/integration/tests/index_variants.rs`. |
| `CREATE INDEX … INCLUDE (cols)` (covering index) | ✅ | `INCLUDE` covering-column list parsed and accepted as metadata; covering-index lookup optimisation deferred to v0.2. Tested in `tests/integration/tests/index_variants.rs`. |
| `CREATE INDEX IF NOT EXISTS` | ✅ | Idempotent `IF NOT EXISTS` guard respected: second creation of an existing index returns success without error. Tested in `tests/integration/tests/index_variants.rs`. |
| Multi-column `CREATE INDEX (a, b, c)` | ✅ | Arbitrary-arity column lists accepted. ASC/DESC/NULLS FIRST/NULLS LAST per-column ordering parsed and silently normalised (storage is order-agnostic in v0.1). Tested in `tests/integration/tests/index_variants.rs`. |
| `CREATE UNIQUE INDEX … INCLUDE (covering)` (unique covering index) | ✅ | Combines UNIQUE flag + INCLUDE covering columns — both flags accepted as metadata together. Tested in `tests/integration/tests/index_variants.rs`. |
| `DROP INDEX [IF EXISTS] name` | ✅ | Removes the catalog index record; no physical file to delete in v0.1. `IF EXISTS` silently succeeds when the index is absent. Tested in `tests/integration/tests/index_variants.rs`. |
