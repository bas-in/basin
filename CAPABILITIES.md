...
| `information_schema` + `pg_catalog` bulk expansion (21 more views) | ✅ | Phase bulk-catalog-views. **pg_catalog:** `pg_database` (one row, current tenant DB), `pg_roles` (alias to pg_authid), `pg_views` (matviews), `pg_indexes` (denormalised PK index info), `pg_tables` (denormalised base-table info), `pg_settings` (8 GUC defaults — server_version/TimeZone/client_encoding/…), `pg_extension` (empty), `pg_description` (empty), `pg_stat_user_tables` (row-count stubs from snapshot), `pg_stat_user_indexes` (scan-count stubs), `pg_locks` (empty — optimistic), `pg_stat_activity` (calling session). **information_schema:** `check_constraints` (populated from field metadata), `triggers` (empty), `sequences` (real — from catalog `list_sequences`), `domains` (real — from catalog `list_domains`), `parameters` (function+procedure args), `role_table_grants` (empty), `user_defined_types` (enum + domain types), `column_domain_usage` (domain-typed columns), `column_udt_usage` (same). `Catalog::list_sequences` trait method added (default: empty; InMemoryCatalog + PostgresCatalog both implemented). Tests: `tests/integration/tests/catalog_views_extras.rs` (21 SELECT + 2 row-count assertions). |
