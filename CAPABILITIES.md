...
| Capability | Status | Notes |
|---|---|---|
| `basin-auth` (signup, signin, magic-link, password reset, email verify, JWT, refresh) | ✅ | Requires SMTP at startup (fail-fast). `auth.users` / `auth.refresh_tokens` live in the engine's own catalog over the loopback pgwire — no separate Postgres to provision. Reserved tenant `basin_auth=<reserved-ulid>` is auto-injected into the tenant resolver. JWT issued + verified per request. `BASIN_AUTH_ENABLED=1`. Optional override `BASIN_AUTH_CATALOG_DSN=<dsn>` points the auth catalog at a separate OLTP store for blast-radius isolation. See [ADR 0005](./docs/decisions/0005-auth-system.md). |
