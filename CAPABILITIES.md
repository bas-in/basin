...
| Window functions — ranking (`row_number`, `rank`, `dense_rank`, `percent_rank`, `cume_dist`, `ntile`) | ✅ | DataFusion-native. All six ranking functions work with `PARTITION BY` + `ORDER BY`. `rank` produces gaps on ties; `dense_rank` does not; `ntile(n)` distributes rows into `n` equal-size buckets. **Divergence from PG:** DataFusion emits `UInt64` for `row_number`/`rank`/`dense_rank`/`ntile`; Basin's df→Arrow bridge does not yet map `UInt64`, so wrap results in `CAST(… AS BIGINT)` when the value is consumed outside DataFusion (matches PG's `bigint` return type). `percent_rank` and `cume_dist` return `Float64` and need no cast. Exercised by `tests/integration/tests/window_fns.rs`. |
| Window functions — offset (`lag`, `lead`, `first_value`, `last_value`, `nth_value`) | ✅ | DataFusion-native. `lag`/`lead` support optional `offset` and `default` arguments (NULL by default). `last_value` / `nth_value` require `ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING` to see the full partition. Exercised by `tests/integration/tests/window_fns.rs`. |
| Window function frame clauses (`ROWS BETWEEN`, `RANGE BETWEEN`, named `WINDOW w AS (…)`) | ✅ | DataFusion-native. `ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW` (running aggregate), `RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING` (partition-wide aggregate), and named window via `WINDOW w AS (PARTITION BY …) … OVER w` all function correctly with aggregate functions (`SUM`, `AVG`, etc.). **Divergence:** `RANK() OVER w` (ranking function with named-window reference) triggers a sqlparser `AS`-keyword disambiguation error in v0.1; use an inline `OVER (PARTITION BY … ORDER BY …)` spec instead. Exercised by `tests/integration/tests/window_fns.rs`. |
| Aggregate functions as window (`sum`, `avg`, `count`, `min`, `max` `OVER (…)`) | ✅ | DataFusion-native. `SUM`/`AVG`/`COUNT(*)`/`MIN`/`MAX` all accept an `OVER (PARTITION BY …)` clause and produce correct per-partition results. Exercised by `tests/integration/tests/window_fns.rs`. |
