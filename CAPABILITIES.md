...
| `SELECT … UNION SELECT …` | ✅ | Set-distinct union via DataFusion; deduplicates rows across both legs. Exercised by `tests/integration/tests/set_ops_ctes.rs::set_op_union_deduplicates`. |
| `SELECT … UNION ALL SELECT …` | ✅ | Bag union; retains duplicate rows. DataFusion-native. Exercised by `tests/integration/tests/set_ops_ctes.rs::set_op_union_all_keeps_duplicates`. |
| `SELECT … INTERSECT SELECT …` | ✅ | Set-distinct intersection; returns only rows common to both legs. DataFusion-native. Exercised by `tests/integration/tests/set_ops_ctes.rs::set_op_intersect`. |
| `SELECT … INTERSECT ALL SELECT …` | ✅ | Bag intersection; respects per-occurrence minimums (multiset semantics). DataFusion-native. Exercised by `tests/integration/tests/set_ops_ctes.rs::set_op_intersect_all`. |
| `SELECT … EXCEPT SELECT …` | ✅ | Set-distinct difference; removes all occurrences of right-leg rows. DataFusion-native. Exercised by `tests/integration/tests/set_ops_ctes.rs::set_op_except`. |
| `SELECT … EXCEPT ALL SELECT …` | 🛠 | Parses and executes, but DataFusion applies set-distinct semantics rather than true bag semantics — all occurrences of a matching value are removed rather than one per right-leg occurrence. PG-correct `{1,2,2,3} EXCEPT ALL {2}` → `{1,2,3}` is returned as `{1,3}`. Exercised (with caveat) by `tests/integration/tests/set_ops_ctes.rs::set_op_except_all`. |
| `WITH name AS (SELECT …) SELECT * FROM name` (CTE) | ✅ | Simple non-recursive CTE; DataFusion inlines the CTE body into the query plan. RLS predicate injection walks CTE bodies so policies apply inside CTEs. Exercised by `tests/integration/tests/set_ops_ctes.rs::cte_simple`. |
| `WITH a AS (…), b AS (…) SELECT … JOIN …` (multiple CTEs) | ✅ | Multiple CTEs defined in one `WITH` clause; each body is planned independently and referenced from the outer query. Exercised by `tests/integration/tests/set_ops_ctes.rs::cte_multiple_with_join`. |
| `WITH RECURSIVE name AS (anchor UNION ALL recursive) SELECT …` | ✅ | Recursive CTE via DataFusion's recursive plan node. Supports `UNION ALL` recursive step with termination predicate. `UNION`-deduplicated recursion falls back to iterative evaluation. Caveat: column-list syntax `WITH RECURSIVE name(col)` is parsed but the alias is not propagated into the schema — use explicit `AS <alias>` on the anchor-leg literals instead. Exercised by `tests/integration/tests/set_ops_ctes.rs::cte_recursive`. |
| `WITH name AS MATERIALIZED (…) SELECT …` (PG-15 hint) | 🛠 | `MATERIALIZED` hint is rejected by sqlparser-rs (0.52/0.53) with a parse error before reaching DataFusion. `NOT MATERIALIZED` is also rejected. Plain bare-CTE form works fully. Will become ✅ once sqlparser gains support. Limitation documented by `tests/integration/tests/set_ops_ctes.rs::cte_materialized_hint`. |
