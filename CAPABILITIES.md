...
| `GENERATED ALWAYS AS IDENTITY` columns | ✅ | Auto-creates a per-table backing sequence `<table>_<col>_seq`; INSERT auto-fills omitted slots via `Catalog::nextval`; user-supplied values rejected with SQLSTATE 428C9 unless `OVERRIDING SYSTEM VALUE` is set. UPDATE on the column also rejected. INT/BIGINT only. Inline sequence options (`START 100`) deferred — use a standalone `CREATE SEQUENCE`. |
| `GENERATED BY DEFAULT AS IDENTITY` columns | ✅ | Same backing-sequence machinery as ALWAYS; user-supplied values accepted by default. `OVERRIDING USER VALUE` discards the user's literal and re-fills from `nextval`. |
| `OVERRIDING SYSTEM VALUE` / `OVERRIDING USER VALUE` on INSERT | ✅ | Extracted by a string-literal- and comment-aware textual pre-screen (sqlparser 0.52 doesn't recognise the clause); kind stashed on the per-session state and consumed once by `exec_insert`. |
| `SERIAL` / `BIGSERIAL` / `SMALLSERIAL` column shorthand | ✅ | Sugar over `GENERATED BY DEFAULT AS IDENTITY`: auto-creates the implicit `<table>_<col>_seq` sequence, forces NOT NULL, INSERT fills via the same identity path. Accepts literal user values. |
