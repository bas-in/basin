...
| `INSERT … ON CONFLICT (col) DO UPDATE SET …` (upsert) | ✅ | Pre-check strategy: existence-check SELECT on the conflict column(s); on hit, synthesises and executes an UPDATE; on miss, falls through to a plain INSERT. Single-row VALUES form; multi-row upsert deferred to v0.2 (would need per-row conflict detection). `ON CONFLICT DO NOTHING` falls through to INSERT (duplicate will surface as a PK constraint error until v0.2 scan-based detection ships). |
| `INSERT INTO t DEFAULT VALUES` | ✅ | Builds a single all-NULL row, applies per-column `DEFAULT` expressions (including `nextval`, `now()`, etc.), then writes via the normal INSERT path. Generated columns are materialised as usual. |
| `INSERT … RETURNING *` | ✅ | After the row is written and committed, the fully-materialised `RecordBatch` (with generated-column values filled in) is returned as `ExecResult::Rows`. Partitioned-table `RETURNING` deferred to v0.2. |
| `UPDATE … WHERE col IN (SELECT …)` | ✅ | The inner SELECT is materialised via DataFusion before the mutation scan; the result set is rewritten to an `IN (lit, …)` predicate that the existing CoW update path evaluates per-file. `WHERE EXISTS (SELECT …)` deferred to v0.2. |
| `DELETE … WHERE col IN (SELECT …)` | ✅ | Same materialise-then-rewrite strategy as `UPDATE … WHERE col IN (SELECT …)`. |
