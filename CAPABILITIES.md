...
| Interval arithmetic (`ts ± interval`, `interval ± interval`, `interval × num`) | ✅ | Phase 5.11.S — `timestamp ± interval`, `interval ± interval`, `interval × numeric` / `interval / numeric`, SQL-standard literals (`INTERVAL '1' DAY`, year-to-month, day-to-second forms). DataFusion-native where possible; shims in `interval_tz_udf.rs` for `justify_days` / `justify_hours` / `justify_interval` / `to_char(interval, fmt)` / `extract_epoch_from_interval`. Exercised by `tests/integration/tests/interval_tz.rs`. |
| `AT TIME ZONE` / `timezone(zone, ts)` | ✅ | Phase 5.11.S — `timestamp AT TIME ZONE 'UTC'` rewritten to `timezone('UTC', ts)` via pre-pass; function form works directly. TIMESTAMPTZ column type supported in DDL + INSERT. `current_timestamp AT TIME ZONE 'UTC'` returns wall-clock time in the target zone. Exercised by `interval_tz.rs`. |
| Date arithmetic (`date ± int`, `date - date`) | ✅ | Phase 5.11.S — `date + integer` / `date - integer` via `date_add_int` / `date_sub_int` shims; `date - date` returns int days via `date_diff_days` shim. `date + interval` returns timestamp natively via DataFusion. `generate_series(date_start, date_end, '1 day'::interval)` works. Exercised by `interval_tz.rs`. |
| `age(ts1, ts2)` (returns interval) | ✅ | Phase 5.11.S — 2-arg form returns native interval type. 1-arg form `age(ts)` (relative to current_date) deferred to v0.2. Exercised by `interval_tz.rs`. |
| Time-source functions (`clock_timestamp` / `transaction_timestamp` / `statement_timestamp` / `localtime` / `localtimestamp`) | 🛠 | Stubs registered alongside `now()` / `current_timestamp` in Phase 5.11.B. Basin's notion of "transaction" / "statement" boundaries does not yet differ from wall-clock — all five currently return wall-clock. Real semantics land when single-shard transactions ship. |
