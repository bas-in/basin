...
| `COPY FROM STDIN` / `COPY TO STDOUT` | ✅ | CSV format (`WITH (FORMAT CSV [, HEADER true/false] [, DELIMITER 'c'] [, NULL 'str'] [, QUOTE 'c'] [, ESCAPE 'c'])`). Column-list `COPY t (c1, c2) FROM STDIN` supported; unlisted columns use their DEFAULT or NULL. Explicit column-list `COPY t (c1, c2) TO STDOUT` supported. Query-source `COPY (SELECT … FROM t WHERE …) TO STDOUT` ✅. File-path `COPY t FROM '/abs/path'` / `COPY t TO '/abs/path'` ✅ gated behind `BASIN_COPY_ALLOW_FILE_PATHS=1` (SQLSTATE 42501 when off) + `BASIN_COPY_PATH_ALLOWLIST=/dir1:/dir2` directory allowlist (SQLSTATE 42601 for out-of-allowlist paths). `COPY … FROM PROGRAM '…'` / `COPY … TO PROGRAM '…'` 🚫 rejected with SQLSTATE 42501. BINARY format rejected with 42601. COPY-IN imports via row-by-row INSERTs; mid-stream errors drain to `CopyDone` before responding (connection stays usable). Drives both simple-query and extended-query so `tokio_postgres::copy_in` / `copy_out` and `psql \copy` both work. |
