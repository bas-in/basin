...
| FTS `@@` operator (`tsvector @@ tsquery`) | ✅ | DataFusion has no native handler for `@@`. `basin-engine::pg_ast::rewrite_tsvector_at_at` lowers every `lhs @@ rhs` to `tsvector_match_udf(lhs, rhs)` in the SQL string before sqlparser sees it. Operand capture is comment / string-literal / quoted-identifier aware and handles parens, function calls, `::tsvector`/`::tsquery` casts (which are stripped — both types map to Utf8), qualified column refs, and multiple `@@`s per statement. `@@@` and other `@`-flavoured operators (`<@`, `@>`, `@?`, `?@`) pass through unrewritten. The target UDF `tsvector_match_udf` is a v0.1 stub returning `true` (match-all); real selectivity deferred to `basin-fts`. |
...
| `TSVECTOR` / `TSQUERY` | ✅ | Both map to Arrow `Utf8` internally (v0.1 stub — no real tokenisation). `CREATE TABLE … (col TSVECTOR)` / `… (col TSQUERY)` accepted; INSERT / SELECT round-trip cleanly. The matching FTS UDFs (`to_tsvector`, `to_tsquery`, `plainto_tsquery`, `phraseto_tsquery`, `websearch_to_tsquery`, `ts_rank`, `ts_rank_cd`, `ts_headline`, `setweight`, `strip`, `tsvector_length`, `numnode`, `querytree`) are stubs returning identity/zero/false. Real tokenisation + selectivity deferred to `basin-fts`. |
...
| Postgres built-in FTS (`tsvector`/`tsquery`/`@@` + `to_tsvector`/`to_tsquery`/`ts_rank`/`ts_headline` family) | 🛠 | v0.1 stubs only — accept-and-execute so ORM migration scripts, generated columns, and SELECT-list expressions don't error. Types map to Utf8; UDFs return identity/zero/false. `@@` rewrites to `tsvector_match_udf` (always-true). Real tokenisation, lexeme ranking, and selectivity ship in `basin-fts` (v0.2 — tantivy sidecar or DuckDB FTS extension; promotion checklist in `crates/basin-engine/src/fts_udf.rs` module docs). |
