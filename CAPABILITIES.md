...
| `CREATE VIEW v AS SELECT …` | ✅ | Plain (non-materialized) views stored in catalog (`Catalog::register_view`); query-rewrite inline subquery at `exec_select` time so DataFusion plans the view body as a derived table. `CREATE OR REPLACE VIEW` replaces the stored definition. `DROP VIEW [IF EXISTS] v [CASCADE\|RESTRICT]` removes it (CASCADE/RESTRICT accepted, no dependents tracked in v0.1). `CREATE TEMP VIEW` / `CREATE TEMPORARY VIEW` accepted (same impl as plain view in v0.1 — in-memory catalog is session-scoped so temporary semantics hold in practice). View columns inherit types from the SELECT. Covered by `tests/integration/tests/views_and_join_dml.rs`. |
| `DELETE FROM t USING u WHERE …` | ✅ | Join-form DELETE. Requires the target table to have a PRIMARY KEY. Engine materialises `SELECT t.pk FROM t, u WHERE <condition>` via DataFusion, then issues a plain `DELETE FROM t WHERE pk IN (…)` using the collected PK values. Covered by `tests/integration/tests/views_and_join_dml.rs`. |
| `UPDATE t SET col = u.col FROM u WHERE …` | ✅ | Join-form UPDATE. Requires a PRIMARY KEY on the target table. Engine materialises `SELECT t.pk, u.col FROM t, u WHERE <condition>` via DataFusion, then issues one `UPDATE t SET col = v WHERE pk = k` per matched row. RHS may be any DataFusion-evaluable expression (column ref, literal, arithmetic). Covered by `tests/integration/tests/views_and_join_dml.rs`. |
