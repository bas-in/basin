...
| `TRUNCATE [TABLE] t [, t2] [CASCADE\|RESTRICT] [RESTART\|CONTINUE IDENTITY]` | ✅ | Real operation: atomically removes all rows via `replace_data_files` (empty added set) + physical object deletion + session refresh. `RESTART IDENTITY` resets `SERIAL`/`BIGSERIAL`/`SMALLSERIAL`-backed sequences via `setval`. `CONTINUE IDENTITY` (default) leaves sequences running. Multi-table TRUNCATE processes tables sequentially; no cross-table rollback in v0.1 (auto-commit). CASCADE/RESTRICT are accepted but not enforced (FK cascade on TRUNCATE deferred to v0.2). |
| Foreign Data Wrappers (syntactic accept-only) | ✅ | `CREATE FOREIGN DATA WRAPPER`, `CREATE SERVER`, `CREATE USER MAPPING`, `CREATE FOREIGN TABLE`, `IMPORT FOREIGN SCHEMA` and their `DROP` counterparts all accepted with empty tags; Basin has no FDW execution engine. ORMs and migration tools that emit FDW DDL (django-postgres-extra, pg_dump restore) will not error. |
| `ALTER DEFAULT PRIVILEGES … GRANT/REVOKE …` | ✅ | Accepted as noop; Basin's real auth comes from `auth.uid()`/`auth.role()` per ADR 0005/0013. |
| `REASSIGN OWNED BY … TO …` / `DROP OWNED BY …` | ✅ | Accepted as noop; ownership is a Postgres concept with no Basin equivalent. |
| `SET CONSTRAINTS [name\|ALL] DEFERRED\|IMMEDIATE` | ✅ | Accepted as noop; Basin is auto-commit with no deferred constraint evaluation in v0.1. |
| `SECURITY LABEL FOR provider ON object IS 'text'` | ✅ | Accepted as noop; security labels not evaluated by Basin. |
