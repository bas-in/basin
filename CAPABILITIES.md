...
| `CREATE SCHEMA [IF NOT EXISTS] name [AUTHORIZATION role]` | ✅ | Per-session schema namespace. `"public"` always exists and `CREATE SCHEMA public` is idempotent. `AUTHORIZATION` clause accepted; owner ignored (Basin uses JWT-based auth, not roles). Schema names stored case-insensitively. Verified by `tests/integration/tests/schema_ddl.rs`. |
| `DROP SCHEMA [IF EXISTS] name [CASCADE\|RESTRICT]` | ✅ | Removes schema from session namespace. `CASCADE` and `RESTRICT` both accepted (Basin's flat per-tenant model has no owned-object graph to cascade). Dropping `"public"` rejected. `IF EXISTS` suppresses the not-found error. Verified by `tests/integration/tests/schema_ddl.rs`. |
| `ALTER SCHEMA name RENAME TO new_name` | ✅ | Renames a schema in the session namespace. Old name must exist; new name must not exist; `"public"` cannot be renamed. `search_path` entries referencing the old name are updated in-place. Recognised via textual pre-screen (sqlparser 0.52 has no `AlterSchema` AST node). Verified by `tests/integration/tests/schema_ddl.rs`. |
| `SET search_path = schema [, …]` | ✅ | Per-session ordered list of schemas consulted when resolving unqualified table names. Both `=` and `TO` assignment forms accepted. Value persists for the lifetime of the connection. Verified by `tests/integration/tests/schema_ddl.rs`. |
| `SHOW search_path` | ✅ | Returns a single-row, single-column result with the current `search_path` as a comma-separated string — real per-session value, not a static stub. Note: a sibling branch (`agent-a0f45b8b5d58608d0`, `noop_accept`) handles `SHOW <var>` generically as a no-op empty; on merge that branch's generic handler must be guarded to not shadow this real `SHOW search_path` path. Verified by `tests/integration/tests/schema_ddl.rs`. |
| Schema-qualified table names (`schema.table`) | ✅ | Two-part `schema.table` names accepted in `CREATE TABLE`, `INSERT`, `SELECT`, `UPDATE`, `DELETE`, `DROP TABLE`, and index DDL. The schema qualifier is validated against the session's schema registry and stripped for catalog lookup (all tables share the tenant's flat namespace). Bare table names continue to work unchanged. Verified by `tests/integration/tests/schema_ddl.rs`. |
