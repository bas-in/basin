...
| `overlaps(s1, e1, s2, e2)` interval predicate | ✅ | P10 — `crates/basin-engine/src/datetime_extras.rs`. PG `OVERLAPS` semantics: two half-open intervals `[s1,e1)` and `[s2,e2)` overlap iff `s1 < e2 AND s2 < e1`. Accepts `Timestamp(Microsecond, _)` or `Timestamp(Nanosecond, _)` (DataFusion coerces `NOW()` to nanosecond); null in any argument propagates as NULL. Returns `Boolean`. |
| `'infinity'::timestamp` / `'-infinity'::timestamp` sentinels | ✅ | P10 — `cast_infinity_timestamp(text)` UDF + SQL pre-rewrite in `executor.rs`. Maps `'infinity'` → `i64::MAX` µs and `'-infinity'` → `i64::MIN` µs (PG's internal sentinel values). Both `::timestamp` and `::timestamptz` cast syntax rewritten before sqlparser sees the SQL. Comparison operators (`>`, `<`, `=`) work naturally on the resulting `Timestamp(Microsecond, None)` column. |
| `date_part(field, ts)` function | ✅ | P10 — alias for DataFusion's built-in `date_part`. Registered so callers using `date_part('year', NOW())` notation (rather than `EXTRACT(year FROM ...)` syntax) get `Float64` results consistent with PG's `double precision` return type. Covered by `tests/integration/tests/datetime_extras.rs`. |
| `array_dims(arr)` function | ✅ | P10 — `crates/basin-engine/src/datetime_extras.rs`. PG `array_dims` semantics: returns a dimension string such as `'[1:3]'` (1-D) or `'[1:2][1:2]'` (2-D). Accepts `List(T)` (1-D) and `List(List(T))` (2-D nested arrays). Higher ranks return an execution error. |
| Multi-dimensional SQL arrays (`int[][]`, `text[][]`, …) | ✅ | P10 — `crates/basin-engine/src/convert.rs` List-arm extension. `List(List(T))` (n-D) now round-trips through the DataFusion ↔ workspace Arrow bridge. `'{{1,2},{3,4}}'::int[][]` literal syntax plans and executes; child values translated recursively preserving offsets and null bitmaps. Enables `array_dims` on 2-D columns and prepares the bridge for future `unnest(arr)` over nested arrays. |
...
| `int[]` / `text[]` / scalar-type 1-D arrays | ✅ | P10 — Arrow `List(T)` in the DataFusion ↔ workspace bridge. `ARRAY[1,2,3]` literals, `ARRAY[...]` constructor, and `'{1,2,3}'::int[]` cast syntax all plan and execute. `array_dims` returns `'[1:N]'`. |
| `int[][]` / multi-dimensional arrays | ✅ | P10 — Arrow `List(List(T))` recursive bridge (`convert.rs`). `ARRAY[[1,2],[3,4]]` constructor syntax round-trips through the engine; `array_dims` returns `'[1:outer][1:inner]'`. Schema translation (`ws_to_df` / `df_to_ws`) and batch conversion both handle arbitrary nesting depth recursively. Note: PG string-cast syntax `'{{1,2},{3,4}}'::int[][]` is not supported by DataFusion's SQL parser for nested arrays; use the `ARRAY[[...]]` constructor instead. |
