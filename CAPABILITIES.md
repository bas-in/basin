...
| `REFRESH MATERIALIZED VIEW <name> [WITH (full = true)]` | ✅ | Textual pre-screen (sqlparser 0.52 has no REFRESH AST node). `WITH (full=true)` forces a full re-materialisation; incremental mode is the default for time-bucketed GROUP BY shapes. |
| `DROP MATERIALIZED VIEW [IF EXISTS] <name>` | ✅ | Textual pre-screen removes the CV catalog entry; underlying data files are not GC'd until the next compaction pass. |
...
| `CREATE TYPE <name> AS ENUM ('a', 'b', …)` | ✅ | Parsed via sqlparser native AST + pg_query AST matcher. Enum labels stored in catalog; column DDL resolves the type name to `Utf8` + `BASIN_ENUM_TYPE` field metadata. INSERT validates every label. |
| `ALTER TYPE <name> ADD VALUE '<label>'` | ✅ | Textual pre-screen (sqlparser 0.52 has no AlterType node). Appends the label to the stored enum definition atomically. No rewrite of existing rows needed (stored as Utf8). |
| `DROP TYPE [IF EXISTS] <name>` | ✅ | Dispatched through sqlparser's native `Statement::Drop { ObjectType::Type }` arm. |
| `CREATE DOMAIN <name> AS <base_type> [CHECK (<predicate>)]` | ✅ | Textual pre-screen + pg_query AST matcher. Domain stored in catalog with base type and optional CHECK predicate. CHECK is evaluated on every INSERT / UPDATE row via DataFusion; SQLSTATE 23514 on violation. |
| `DROP DOMAIN [IF EXISTS] <name> [CASCADE\|RESTRICT]` | ✅ | Textual pre-screen + pg_query AST matcher. `CASCADE` / `RESTRICT` keywords accepted and ignored (v0.1 has no cross-object dependency graph). |
| `CREATE SEQUENCE [IF NOT EXISTS] <name> [START … INCREMENT … MINVALUE … MAXVALUE … CACHE … CYCLE]` | ✅ | Full PG option grammar via textual pre-screen (sqlparser 0.52 only parses one option per statement). All six sequence options parsed and applied; `TEMPORARY` rejected with a clear error. |
| `ALTER SEQUENCE [IF EXISTS] <name> RESTART [WITH <n>]` | ✅ | Textual pre-screen. `RESTART [WITH n]` resets the sequence counter via the catalog `setval(advance=false)` path. `OWNED BY` accepted and ignored. Structural option changes (INCREMENT, MINVALUE, etc.) produce a clear "not yet supported" error pointing to the DROP+CREATE workaround. |
| `DROP SEQUENCE [IF EXISTS] <name>` | ✅ | Dispatched through sqlparser's native `Statement::Drop { ObjectType::Sequence }` arm. |
| `nextval('<name>')` | ✅ | SQL-string rewriter resolves `nextval('seq')` to a BIGINT literal before sqlparser runs; dynamic (non-literal) sequence-name arguments fall through to the tombstone UDF with a clear error. |
| `currval('<name>')` | ✅ | Rewriter resolves `currval('seq')`. Per-session cache ensures `currval` reflects only this session's last `nextval`. SQLSTATE 55000 when `nextval` has not been called yet in the session. |
| `setval('<name>', n [, advance])` | ✅ | Rewriter resolves two- and three-argument forms. `setval(seq, n, true)` (default) advances the catalog counter and arms `currval`; `setval(seq, n, false)` makes the next `nextval` return exactly `n`. |
| `lastval()` | ✅ | Rewriter resolves `lastval()` to the most recent value returned by any `nextval` call in the current session. SQLSTATE 55000 when no `nextval` has been called in the session (matches PG). |
