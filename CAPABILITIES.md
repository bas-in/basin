...
| `jsonb_typeof(jsonb)` | ✅ | Returns `"object"`, `"array"`, `"string"`, `"number"`, `"boolean"`, or `"null"`. Accepts both `LargeBinary` (stored JSONB column) and `Utf8` (string literal) inputs. Registered in `crates/basin-engine/src/jsonb_udf.rs`; exercised by `tests/integration/tests/jsonb_udfs.rs`. |
| `jsonb_pretty(jsonb)` | ✅ | Returns indented UTF-8 text of the JSON value — multi-line with sorted keys (serde_json canonical form). Accepts `LargeBinary` and `Utf8`. |
| `jsonb_array_length(jsonb)` | ✅ | Returns the number of elements in a JSON array (Int64). Returns 0 for non-array inputs (matches PG behaviour). |
| `jsonb_strip_nulls(jsonb)` | ✅ | Recursively removes null-valued keys from objects. Arrays are traversed but null *elements* are kept (matches PG). Returns `LargeBinary`. |
| `jsonb_set(target, path, new_value [, create_missing])` | ✅ | Replace (or create) the value at a path given as a Postgres array-literal `'{key1,key2}'` or dot-separated string. 4th argument `create_missing` defaults to `true`. Returns `LargeBinary`. |
| `jsonb_insert(target, path, new_value [, insert_after])` | ✅ | Insert a new value at a path; for array targets `insert_after` controls before/after (default `false` = before). Object keys are set unconditionally. Returns `LargeBinary`. |
| `jsonb_path_query(target, jsonpath)` | ✅ | Navigate to the node at a simple dotted JSONPath (e.g. `$.user.name`). Returns `LargeBinary` (the matched value) or NULL when the path does not exist. SRF multi-match deferred; first match returned. |
| `jsonb_path_exists(target, jsonpath)` | ✅ | Returns `true`/`false` (Boolean). Same path-navigation logic as `jsonb_path_query`; NULL when either argument is NULL. |
| `jsonb_path_match(target, jsonpath)` | ✅ | Alias of `jsonb_path_exists` for simple paths (full predicate-match expressions deferred to v0.2). |
| `jsonb_object_keys(jsonb)` | ✅ | SRF stub: returns comma-joined key names as `Utf8`. True `SETOF text` expansion requires table-function plumbing not yet in DataFusion's scalar-UDF framework; deferred to v0.2. |
| `jsonb_each(jsonb)` | ✅ | SRF stub: returns the whole JSON object serialised as `Utf8`. Useful for inspecting stored objects; full row-set expansion deferred to v0.2. |
| `jsonb_each_text(jsonb)` | ✅ | SRF stub: returns `key=value,...` pairs as `Utf8` (string-coerced values). Same v0.2 caveat as `jsonb_each`. |
| `jsonb_array_elements(jsonb)` | ✅ | SRF stub: returns the first array element as `LargeBinary`. Full element expansion deferred to v0.2. |
| `jsonb_array_elements_text(jsonb)` | ✅ | SRF stub: returns the first array element as `Utf8` (string-coerced). Same v0.2 caveat. |
| `jsonb_build_object(key, val, …)` | ✅ | Variadic key/value pairs → JSONB object (`LargeBinary`). Odd number of arguments returns an error. Keys are coerced to strings; values accept `Utf8`, `Int64`, `Boolean`, or `LargeBinary` (JSONB). |
| `jsonb_build_array(val, …)` | ✅ | Variadic values → JSONB array (`LargeBinary`). Accepts same types as `jsonb_build_object` values. |
| `to_jsonb(any)` | ✅ | Converts any scalar to its JSONB representation (`LargeBinary`). Integers → JSON number; text → JSON string; booleans → JSON boolean; JSONB passthrough. |
| `row_to_json(record)` | ✅ | Stub that converts any scalar argument to JSONB (`LargeBinary`). Full record-to-object conversion requires composite type support (deferred to v0.2). |
| `array_to_json(array [, pretty])` | ✅ | Accepts a JSON array literal or a JSONB column and returns it as `LargeBinary`; non-array inputs are wrapped in a single-element array. `pretty` argument accepted but ignored (use `jsonb_pretty` instead). |
| `jsonb_agg(any)` stub | ✅ | Registered; returns a clear user-friendly error directing callers to `jsonb_build_array`. True `AggregateUDFImpl` accumulator deferred to v0.2. |
| `jsonb_object_agg(key, value)` stub | ✅ | Registered; returns a clear user-friendly error directing callers to `jsonb_build_object`. True `AggregateUDFImpl` accumulator deferred to v0.2. |
