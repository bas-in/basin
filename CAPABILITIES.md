...
| Realtime SSE (`GET /realtime/v1/sse/:project/:table`) | ✅ | Phase 5.11.R2 (`800d6d2`). Row-change stream over Server-Sent Events per table; bearer-JWT auth; compatible with `EventSource`. Now fed by **hot-tier UPDATE/DELETE** as well as cold copy-on-write and INSERT: the point-mutation fast paths and in-transaction hot mutations (drained at COMMIT in order; dropped on ROLLBACK) dispatch before/after change events — capture is gated on a post-commit sink being attached, so the no-CDC OLTP hot path is unaffected. |
...
| `UPDATE` / `DELETE` | ✅ | Copy-on-write Iceberg v2. Single-scan partition; `replace_data_files` with optimistic concurrency on both catalog backends; physical deletion of replaced Parquet files. Rewrites encode with the Fast cascade and skip before/after event capture when no sinks / audit / generated columns / RETURNING consume it (`7934575`) — background compaction keeps the Best cascade. `UPDATE … FROM` is **set-oriented and scale-invariant** in target table size: ONE join projects every matched row's full post-image and writes it in ONE batched overlay write (budget-decline falls to a chunked cold drain), so a fixed-match UPDATE…FROM costs the same at 10k and 100k rows (`update_from_scaling_gate`, ratio ≤ 3) — replacing a per-row `UPDATE … WHERE pk = X` re-execution loop that was O(matched·files). A qualified self-referencing SET right-hand side (`SET col = "t"."col" + 1` — the Django `F()` expression shape) resolves by stripping the table qualifier (`orm_sql_shapes.rs`). |
| `ALTER TABLE` | ✅ | `ADD COLUMN` (incl. array / `NOT NULL` with a `DEFAULT` — the default supplies the value, the Django/Rails add-not-null-column shape), `ADD CONSTRAINT <n> CHECK (...)`, `ADD CONSTRAINT <n> UNIQUE (cols)` (backfill-validated against existing rows — duplicates reject with SQLSTATE 23505 — then enforced on every subsequent INSERT/UPDATE; the Django `unique_together` migration shape), `ADD CONSTRAINT <n> FOREIGN KEY (cols) REFERENCES …` (registered in catalog metadata + enforced on subsequent writes; the Django/Rails add-FK-after-create migration shape; existing-row backfill deferred), `DROP CONSTRAINT`, `SET cold_after`, `SET cold_age_column`, `SET BLOOM FILTERS ON`, `SET row_group_rows`, `RESET row_group_rows`, `CLUSTER BY (...)`, `RESET CLUSTER BY`, `ENABLE/DISABLE ROW LEVEL SECURITY`, `CREATE POLICY`, `DROP POLICY` |
...
| `INSERT … ON CONFLICT DO {NOTHING,UPDATE}` | ✅ | `DO NOTHING` suppresses UNIQUE violations on the conflict-target match; `DO UPDATE SET … = EXCLUDED.col` resolves `table.col` + `EXCLUDED.col` references. **Composite targets** (`ON CONFLICT (a, b)`) against composite PK or multi-column UNIQUE constraint shipped (`composite_on_conflict.rs`): DO UPDATE, DO NOTHING, EXCLUDED refs, ON CONFLICT ON CONSTRAINT by name (including implicit `<table>_pkey`), mismatch → SQLSTATE 42P10. NULL semantics: NULLs in composite key do not produce a conflict (PG parity). Multi-row VALUES with mixed conflict/no-conflict rows. |
| `UPDATE … SET col = <expression>` | 🛠 | RHS must be a literal or a single bind parameter (`SET name = $1`). Expressions like `SET name = name \|\| ' (updated)'` or `SET count = count + 1` get rejected with `expected literal of type Utf8 …`. Compute the new value client-side. RMW `SET col = <expr>` fast path handles common patterns (e.g. `SET count = count + 1` via overlay); full arbitrary SQL expression RHS deferred to v0.2. |
| `MERGE INTO … USING … ON … WHEN MATCHED … WHEN NOT MATCHED …` | ✅ | Phase 5.X. Each per-row WHEN action compiled to ordinary INSERT/UPDATE/DELETE through the normal statement pipeline. Actions: WHEN MATCHED THEN UPDATE, WHEN MATCHED THEN DELETE, WHEN MATCHED THEN DO NOTHING, WHEN NOT MATCHED THEN INSERT. First-match-wins clause ordering enforced. Duplicate-target 21000 error raised when a MERGE source row would match the same target row twice. Atomicity: a failing action rolls back the whole MERGE. RLS enforced on every action. VALUES and SELECT-subquery sources both supported. Command tag `MERGE N`. Pinned by `merge_into.rs`. |
| `DECLARE … CURSOR FOR …` / `FETCH` / `MOVE` / `CLOSE` | ✅ | Session-scoped cursors materialised at DECLARE time. `FETCH N FROM c`, `FETCH NEXT`/`PRIOR`/`FIRST`/`LAST`/`ALL`/`ABSOLUTE`/`RELATIVE` directions; `MOVE N FROM c` position-only; `CLOSE c` / `CLOSE ALL`. Past-end FETCH returns 0 rows (not an error). Unknown cursor → SQLSTATE 34000. `DECLARE … WITH HOLD` → SQLSTATE 0A000 (session-scoped in v0.1). Django/psycopg2 server-side cursor pattern (BEGIN + DECLARE NO SCROLL + chunked FETCH + CLOSE + COMMIT). `BASIN_CURSOR_MAX_ROWS` row-cap enforcement. Pinned by `cursor_lifecycle.rs` + `cursor_extended.rs`. |
...
| PostgreSQL array functions and operators | ✅ | Phase 5.11.Q. DataFusion 44 ships `datafusion-functions-nested` with `nested_expressions` enabled by default, providing: constructors (`ARRAY[…]` 1-D, `make_array(make_array(…))` nested); accessors/size (`array_length`, `cardinality`, `array_ndims`, `array_dims`); subscript via `array_element(arr, n)` (1-based); slice via `array_slice(arr, lo, hi)`; mutators `array_append`, `array_prepend`, `array_cat` / `array_concat`, `array_remove`, `array_replace`, `array_position`, `array_positions`; membership `array_has` (= ANY semantics), `array_has_all` (= ALL / @> semantics), `array_has_any` (&& overlap semantics); operators `@>` / `<@` type-discriminated (List types → `array_has_all`; range types handled separately); set-returning `unnest`; `array_to_string` / `string_to_array`. `array_repeat` is the DataFusion equivalent of PG `array_fill`. Array **column types** (`TEXT[]`, `VARCHAR(n)[]`, `INT[]`, `BIGINT[]`, `DOUBLE PRECISION[]`, `BOOLEAN[]`) are supported end-to-end: `CREATE TABLE` and `ALTER TABLE ADD COLUMN` DDL (Arrow `List`), INSERT via both the `ARRAY[…]` constructor and the PostgreSQL curly text form (`'{a,b}'` / `'{}'`, quoted elements + `NULL`), and result columns render on the wire as PG array text `{…}` (psycopg / lib-pq / node-postgres parse them). Gap: `array_lower` / `array_upper` have no native UDF (always 1 / `array_length` for 1-based arrays). Verified by `tests/integration/tests/array_fns.rs`. |
| JSONB GIN indexing (`CREATE INDEX … USING gin`) | ✅ | Phase 5.19. `CREATE INDEX … USING gin (col)` and `CREATE INDEX … USING gin (col jsonb_path_ops)` parse and persist the opclass in the catalog. `crates/basin-engine/src/index_probe.rs` maintains an in-RAM posting list per `(table, col)` and implements containment probe (`@>`, `<@`) with posting-list file pruning; dedup-(term, file) storage makes posting lists survive 1M-row backfills. **Measured (2026-06-11 integrity run):** GIN effectiveness at 10k: `@>` 10.8 ms → 2.52 ms (4.3×, ≈ PG's 2.35 ms with GIN). At 1M, effectiveness 1.12× (269 → 240 ms) while hot-tier overlays from earlier card shapes are live (card-order interaction, correctness-first by design). `BASIN_GIN_POSTING_BUDGET` shared with FTS GIN; over-budget eviction degrades per-file (correct, unpruned), not whole-column. |
| Full-text search (`tsvector` / `tsquery` / `@@`) | ✅ | Phase 5.20. `TSVECTOR` and `TSQUERY` column types (Arrow `Utf8` + `BASIN_TYPE` sidecar metadata); `to_tsvector([config,] text)` with Snowball English stemming + stop-word filtering; `to_tsquery` (Snowball-stemmed for PG parity — `to_tsquery('english','runs')` matches `running`; raw `::tsquery` casts stay unstemmed), `plainto_tsquery`, `phraseto_tsquery`, `websearch_to_tsquery`; `@@` match operator; `ts_rank` TF-weighted; `ts_rank_cd` cover-density (simplified deterministic score); `ts_headline` fragments with `MaxFragments` and `<b>…</b>` wrapping; `setweight(tsvector, 'A'|'B'|'C'|'D')` annotation; `strip()` position removal; `tsvector_length`. **GIN-on-tsvector** (`CREATE INDEX … USING gin (col)`): in-RAM lexeme posting list + CREATE INDEX backfill (settles live overlay first); `@@` probes structurally (`&`/`<->` intersect, `|` unions, `!`/unknown lexemes decline to full scan); provably-empty short-circuit + per-file completeness guard; `BASIN_GIN_POSTING_BUDGET` shared with JSONB GIN. Caveats: per-class weight scoring in `ts_rank` beyond setweight annotation deferred; language configs beyond `english`/`simple` deferred. Full conformance suite: `fts_conformance.rs` (all slices live, including adversarial pruning tests). |
...
| Advisory locks + deadlock detection | ✅ | Phase 5.26+ (ADR 0026). `pg_advisory_lock(key)`, `pg_advisory_unlock(key)`, `pg_advisory_unlock_all()`, `pg_try_advisory_lock(key)` (session-scoped); xact-scoped variants (`pg_advisory_xact_lock`, `pg_advisory_xact_unlock` auto-released at COMMIT/ROLLBACK). Reentrancy: N locks need N unlocks. Project-isolated: same numeric key in two projects does not contend. **Deadlock detection**: wait-for-graph detector (`bounded_lock_wait`) breaks ABBA cycles promptly — one session gets SQLSTATE 40P01 (`deadlock_detected`), the other acquires. Lock-wait timeout: SQLSTATE 55P03 (`lock_not_available`). Disconnect releases all session-held locks. Used internally for ORM migration tooling compatibility (Prisma, sqlx, Diesel, golang-migrate emit `pg_advisory_lock` sequences). Pinned by `advisory_locks.rs`. |
...
| `POINT` (geospatial) | ✅ | `POINT` column DDL type; `ST_MakePoint`, `ST_X`, `ST_Y`, `ST_Distance`, `ST_DWithin`, R-tree index. See Postgres-extension equivalents → PostGIS. |
...
| `NUMERIC` / `DECIMAL` | ✅ | Arrow `Decimal128(p, s)` (1 ≤ p ≤ 38, 0 ≤ s ≤ p). DDL accepts `NUMERIC`, `NUMERIC(p)`, `NUMERIC(p, s)`, `DECIMAL(...)` synonym. Wire format: text **and binary** — the PG varlena-shaped binary NUMERIC encoding is implemented (`encode_numeric_binary` in `basin-router/src/types.rs`, wired into the Decimal128 result branch + ParameterDescription), so binary-mode drivers get native NUMERIC. pgwire OID 1700; `information_schema.columns.data_type` = `"numeric"`. |
| Range types (`int4range`, `int8range`, `numrange`, `tsrange`, `tstzrange`, `daterange`) | ✅ | Phase 5.24. All six range types implemented as JSON-encoded structs (Arrow `Utf8` with `BASIN_TYPE=INT4RANGE` etc. sidecar). Constructors (`int4range(lo, hi [, bounds])`) and operators (`@>`, `<@`, `&&`, `<<`, `>>`, `&<`, `&>`, `-|-`, `+`, `*`, `-`, `range_merge`) registered as DataFusion scalar UDFs in `crates/basin-engine/src/range_udf.rs`; infix operator syntax rewritten to UDF calls via `rewrite_range_operators`. Accessors: `lower`, `upper`, `isempty`, `lower_inc`, `upper_inc`, `lower_inf`, `upper_inf`. Multirange: `int4multirange` construction and `@>` containment. Empty-range semantics (canonical half-open normalization for discrete types). Unindexed range queries fall back to full DataFusion scan; GIST index deferred. `EXCLUDE USING gist` constraints on range columns: parsed, stripped from DDL, and enforced via a sentinel CHECK mechanism that routes to the exclusion enforcer at INSERT time (`extract_exclude_using_gist` in `ddl.rs`, wired in `executor.rs`). Conformance: `range_conformance.rs` (all slices live). |
| `citext` (case-insensitive text) | ✅ | Phase 5.30. `CITEXT` column type implemented: Arrow `Utf8` + `BASIN_TYPE=CITEXT` metadata; case-folding via `citext_fold()` in `crates/basin-common/src/types/citext.rs`. Comparison operators (`=`, `<>`, `<`, `<=`, `>`, `>=`) wired as DataFusion ScalarUDFs (`citext_eq`, `citext_ne`, `citext_lt`, etc.) in `crates/basin-engine/src/operators/citext_cmp.rs`. UNIQUE constraint enforcement folds the key at index time via `crates/basin-storage/src/index/btree_citext.rs`. WHERE-clause auto-folding **shipped**: `CitextAnalyzerRule` (`crates/basin-engine/src/citext_analyzer.rs`, registered in `session.rs`) rewrites `=`, `<>`, `<`, `<=`, `>`, `>=`, `LIKE`, and `ORDER BY` on citext columns to the case-folded form, so `WHERE email = 'FOO@BAR.COM'` matches a citext column with no explicit `::citext` cast or `citext_eq` call. End-to-end harness `tests/integration/tests/citext_harness.rs` is live and green (4 slices: type round-trip, case-insensitive comparison, UNIQUE case-folding, ORM compat). |
| `INTERVAL`, `MONEY`, `XML` | 🚫 | |
| geometric (`LINESTRING`/`POLYGON`/`MULTI*`/`GEOMETRYCOLLECTION`) | ✅ | Full value-level support via `basin-geo` (WKB/EWKB/WKT/GeoJSON codecs + measures + exact topology predicates). Carried as `BYTEA`/WKB — **not** a native column DDL type; only `POINT` has a column type + R-tree index. 3-D geometry, `CIRCULARSTRING`, `CURVEPOLYGON`, `TIN` are not supported. |
...
| WAL (local/object-store backed) | ✅ | `LocalWal` is the default production path (`BASIN_WAL_MODE=local`, byte-identical to single-node) and `basin-server` can mirror the S3-compatible storage backend for WAL object storage. `RaftWal` implements the same trait with openraft over a disk-persisted log/vote/snapshot and is server-selectable via `BASIN_WAL_MODE=raft`: durability becomes a quorum ack, the raft leader is the write fence, and manifest-anchored snapshots bound the log by the flush window (3-node + leader-failure + quorum-commit + failover-drill tests). Cross-process gRPC transport is the `raft-net` feature (`BASIN_RAFT_BIND` / `BASIN_RAFT_PEERS`); v1 is plaintext on an assumed-private cluster network (mTLS is the documented production follow-up). See `crates/basin-wal/RAFT.md`. |
...
| Per-project secondary indexes (B-tree) | ✅ | Phase 5.7 B1 (`33a8162`). `CREATE INDEX <name> ON <table> (<col>)` — fast-select probe path; `pg_index` view populates with real rows. Biggest point-query win for true random ids. **Caveat:** the in-RAM index is legitimately incomplete (built async, FIFO-evicted, blind to rows predating it), so a probe MISS means "unknown", not "empty" — a miss falls through to the pruned cold read so an auto-built index can never make a live row invisible (soundness fix in `f5d1d6f`). DELETE/UPDATE by PK on a single-column-B-tree-indexed table now takes the **hot-tier overlay fast path** (no cold rewrite): the allowlist probe is overlay-guarded (declines while an overlay is live) and `materialize_overlay_for_table` re-registers the replacement file's locations on drain, so the HIT prune stays complete — adversarial oracle `btree_overlay_delete.rs`. |
...
| IVF-flat indexes | ✅ | Native coarse quantiser in `basin-vector::IvfFlatIndex` (deterministic Lloyd k-means into `lists` cells + `ivfflat.probes` nearest-cell scan). `CREATE INDEX … USING ivfflat (col <opclass>) WITH (lists = N)` accepted; `lists` round-trips through the catalog opclass. Approximate by design — candidates are exact-ranked so returned rows are exactly ordered; recall rises with `probes` (pgvector semantics). Measured recall@10 on a seeded clustered set: probes=1 ≈ 0.90, probes=4 = 1.00, all-cells = 1.00 (exact). |
| `vector_avg(v)` aggregate | ✅ | Element-wise mean UDAF (f64 accumulation, f32 result). PG NULL semantics (skips NULL rows; zero rows → NULL); dimension mismatch errors. Registered as `vector_avg` only — not `avg`, which would clobber numeric AVG. |
| `halfvec(N)` column type | ✅ | f32-backed (same `FixedSizeList<Float32>` layout as `vector(N)`); all vector paths work unchanged. Precision note: Basin keeps full f32 precision (no f16 truncation), so distances are at least as accurate as pgvector halfvec. Native f16 on-disk storage deferred (storage-format-version item). |
| `sparsevec(N)` / vector `bit` types | 🚫 | Typed `0A000` (feature_not_supported) — no silent dense coercion. Sparse CSR layout + kernels roadmap-tracked. |
...
| `PostGIS` (subset) | **`basin-geo`** | ✅ | **POINT**: `ST_MakePoint`, `ST_X`, `ST_Y`, `ST_Distance` (Haversine WGS84), `ST_DWithin`, `ST_SRID`, `ST_SetSRID`, `ST_Transform` (SRID-based), `ST_GeogFromText`/`ST_GeographyFromText` aliases. **GeoJSON I/O**: `ST_AsGeoJSON` / `ST_GeomFromGeoJSON`. **General 2-D geometry surface** (all types as WKB/EWKB values — only `POINT` is a native column DDL type): `LINESTRING` / `POLYGON` (+holes) / `MULTIPOINT` / `MULTILINESTRING` / `MULTIPOLYGON` / `GEOMETRYCOLLECTION` via variable-length WKB/EWKB/WKT/GeoJSON codecs; `ST_GeomFromText` / `ST_GeomFromGeoJSON` / `ST_GeomFromWKB`, `ST_AsText` / `ST_AsGeoJSON` / `ST_AsEWKB`; accessors/measures `ST_GeometryType`, `ST_NumPoints`, `ST_NumGeometries`, `ST_GeometryN`, `ST_PointN`, `ST_StartPoint` / `ST_EndPoint` / `ST_ExteriorRing`, `ST_Length` / `ST_Area` / `ST_Perimeter` (planar + WGS84-`geography`), `ST_Centroid`, `ST_Envelope`; exact-planar predicates `ST_Intersects` / `ST_Contains` / `ST_Within` / `ST_Disjoint` / `ST_Crosses` / `ST_Touches` / `ST_Overlaps` (via `geo` crate topology traits — not bbox approximations); `ST_MakeEnvelope`, `ST_Buffer`. **R-tree index on POINT**: `&&` bbox-overlap and `ST_DWithin` radius queries prune to candidate row groups before decoding; `&&` does a single-decode residual. Measured: `ST_DWithin` ~1.7× faster than PostGIS GIST; ~28× faster than unindexed Basin scan at 1M rows. **Honest losses**: `&&` bbox-count and KNN `<->` LIMIT still trail PostGIS GIST by ~150×. R-tree covers POINT only; no GIST index on general geometries. `sparsevec` (3-D geometry) not on roadmap; `ST_Buffer`/`ST_Union` constructive ops not supported. Conformance: `postgis_conformance.rs`. |
| `pg_trgm` | **`basin-trgm`** | ✅ | `similarity(text, text)`, `word_similarity(text, text)`, `show_trgm(text)` SQL UDFs. Operators: `%` (similarity threshold), `<%` (word-similarity threshold), `<->` (distance). GUC overrides: `SET pg_trgm.similarity_threshold`, `SET pg_trgm.word_similarity_threshold`. **GIN trigram index** (`CREATE INDEX … USING gin (col gin_trgm_ops)`): in-RAM posting list per `(table, col)` seeded at INSERT + via CREATE INDEX backfill; `%` / `<->`-backed probes prune to candidate files using the `trgm_min_shared` conservative bound, then re-evaluate `similarity()` on survivors. Differential correctness pinned against unindexed twin by `trgm_gin_index.rs`. SQL surface conformance in `trgm_sql_conformance.rs`. |
| `TimescaleDB` continuous aggregates | **`basin-cv`** | ✅ | `CvSpec` + `CvRefresher::tick`; refresh_interval enforced; per-project materialization. **Incremental refresh shipped** for the `date_trunc(_, col)` / `time_bucket(_, col)` GROUP BY shape — only re-aggregates rows newer than the watermark plus the last partial bucket. Bodies without a detectable time-bucket fall back to full re-execution. `REFRESH MATERIALIZED VIEW … WITH (full=true)` opt-out for explicit full rebuild. `add_continuous_aggregate_policy` / `refresh_continuous_aggregate` (windowed upsert). `drop_chunks` on source preserves materialised rows. SQL surface ships in `basin-engine::cv_ddl`. Full `cagg` harness: `timescale_caggs.rs`. |
| `TimescaleDB` hypertables | **`basin-cv`** | ✅ | `create_hypertable(table, time_column)` accepted; `time_bucket(interval, ts)` all forms (epoch-aligned + `origin`-aligned) registered as DataFusion UDFs; `first(value, ts)` / `last(value, ts)` aggregates (order-by semantics, NULL-skipping); `drop_chunks('table', older_than => ...)` actual deletion; `add_retention_policy` + `run_retention_policy` (delegates to drop_chunks); `timescaledb_information.chunks` + `timescaledb_information.hypertables` catalog views. `time_bucket_gapfill(interval, ts)` + `locf()` carry-forward shipped (`timescale_gapfill.rs`). Conformance in `timescale_conformance.rs` + `timescale_completions.rs` + `timescale_caggs.rs` + `timescale_gapfill.rs`. |
...
| `citext` | native `citext` type | ✅ | Phase 5.30. Type, comparison operators, UNIQUE folding, and WHERE-clause / ORDER BY auto-folding (`CitextAnalyzerRule`) shipped (see Types table); end-to-end harness `citext_harness.rs` live + green. No `CREATE EXTENSION citext;` needed — always available. |
...
| `home_region` per project + `WrongRegion` typed error | ✅ | `ProjectMetadata::home_region`; writes to a non-home replica reject with the typed retryable `WrongRegion` error (SQLSTATE 40001) so the client can retry on the home region. Pinned by `multiregion_homing.rs`. |
| `basin.read_tier` GUC (lagging reads) | ✅ | `SET basin.read_tier = 'lagging'` allows reads from a non-home replica without a WrongRegion reject; the replica serves from its local catalog state. Default is `'primary'`. |
| `WriteForwarder` seam | ✅ | Pluggable trait in `basin-engine`; a proxy node can install a forwarder that routes writes to the home region's engine. OSS ships the seam; the cloud proxy wires the implementation. |
| `BASIN_LEASE_MODE=required` write-fence | ✅ | Multi-node phase 1 (ADR 0023). Shard acquires writer lease via catalog CAS on first touch; heartbeat renews across TTL windows (renew is not a regrant); on lease loss in-flight and fresh handles refuse writes with `BasinError::LeaseNotHeld`; reads continue; replica recovers by re-acquiring at a higher epoch. Off mode (default): pre-existing `CommitConflict` from `get` when a peer holds the lease. Pinned by `lease_mode_required.rs` + `lease_failure_paths.rs` + `lease_handoff.rs`. |
| Quorum-replicated WAL (`BASIN_WAL_MODE=raft`) | ✅ | `RaftWal` with openraft 0.9. Disk-backed log + vote + snapshot (`raft_storage.rs`). Manifest-anchored snapshots (small manifest pointer, not a data copy; log stays bounded by the un-flushed window). `raft-net` feature: cross-process gRPC transport via tonic (lazy-dial, HTTP/2 channel per peer, backoff on eviction, `BASIN_RAFT_BIND` / `BASIN_RAFT_PEERS` / `BASIN_RAFT_BOOTSTRAP`). V1 is plaintext on assumed-private cluster network; mTLS is documented follow-up. Raft leadership is the write fence and supersedes `BASIN_LEASE_MODE`. Cluster status logged at startup. Failover drill tests in `wal_durability.rs` + `wal_tx_commit_marker.rs`. |
| Per-tier connection ceiling | ✅ | Hard ceiling on simultaneously open pgwire connections per project. Enforced at pgwire startup; refused with SQLSTATE 53300. `CatalogConnectionLimitProvider` reads from catalog (fail-closed: no stored ceiling → 25, the Free tier). Set via `POST /admin/v1/projects/:id/max-connections`. RAII guard decrements on disconnect; lowering ceiling never kills existing connections. |
...
| Rate limiting (pgwire side) | ✅ | per-project token-bucket via `governor` (same crate as basin-net). Default off; `BASIN_PGWIRE_RATE_LIMIT_QPS=100` sets the global default (100 qps sustained / 300 burst), with bucket-empty mapped to Postgres SQLSTATE `53400` (`configuration_limit_exceeded`). Per-project (per-tier) overrides are catalog-driven (`Catalog::{get,set}_project_rate_limit_qps`), resolved lazily on a project's first query and cached; the control plane pushes them via `POST /admin/v1/projects/:id/rate-limit`. Disabled by default is a true no-op (no per-query cost). |
...
| Capability | Status | Notes |
|---|---|---|
| Durable CDC ring (Phase 1) | ✅ | `basin-cdc`: `CdcRingWriter` implements `ChangeEventSink` as a **post-commit** sink. Every committed mutation (including HTAP hot-tier UPDATE/DELETE fast paths) is appended to a per-project append-only ring on object storage. Committed-only and commit-ordered (`seq` monotonic per project). Batched with size/time flush; crash-loss window is the commit→PUT window (at-least-once). Retention: `BASIN_CDC_RETENTION_HOURS` (default 24h, max 168h). Cursor-expired frame when subscriber lags past retention. |
| SSE stream with resumable cursor (Phase 1) | ✅ | `basin-cdc::live`: `GET /cdc/v1/sse/:project` streams events with a resumable `seq` cursor via Server-Sent Events. Bearer-JWT auth. |
| Webhook fanout (Phase 2) | ✅ | `basin-webhooks`: disk-backed retry queue keyed by `(subscription_id, project, table, seq)` SHA-256 idempotency key. Exponential backoff (1s → 5min cap), `max_retries` cap, dead-letter file. Auto-pause after `WebhookConfig::auto_pause_after` (default 24h); resume explicit. Per-project rate limiting via `basin-net` token bucket. |
| Kafka/Redpanda push sink (Phase 3) | ✅ | `basin-cdc::kafka`: delivery worker that drains the CDC ring into a Kafka/Redpanda topic. |
| pgoutput / logical replication protocol | 🚫 | Not the right shape for object-store storage; CDC ring + SSE + webhooks + Kafka are the Basin equivalent. `cdc_pgoutput_harness.rs` is a test-first gate for future Debezium/Fivetran connector work. |

## Client SDKs

| SDK | Language | Status | Notes |
|---|---|---|---|
| `basin-js` | TypeScript | ✅ | Node, Bun, Deno, Cloudflare Workers, browser. Auth, query, REST, realtime (SSE + WebSocket), blob storage, WASM functions. Arrow IPC streaming (`Accept: application/vnd.apache.arrow.stream`). |
| `basin-python` | Python | ✅ | Async (`AsyncBasinClient`) + sync (`BasinClient`). Auth, query, REST, realtime WebSocket (`realtime` extra, websockets), blob storage, functions. Arrow IPC via pyarrow (`arrow` extra). |
| `basin-go` | Go | ✅ | Auth, query, REST, realtime, storage, functions. Arrow IPC via `arrow-go/v18`. |
| `basin-java` | Java | ✅ | Auth, query, REST, storage, functions. Gradle build. |
| `basin-rust` | Rust | ✅ | Auth, query, REST, realtime, storage, functions. |
| `basin-ruby` | Ruby | ✅ | Auth, query, REST, storage, functions. Gem. |
| `basin-dotnet` | C# / .NET | ✅ | Auth, query, REST, storage, functions. .sln. |
| `basin-php` | PHP | ✅ | Auth, query, REST, storage, functions. Composer. |
| `basin-dart` | Dart / Flutter | ✅ | Auth, query, REST, storage, functions. Pub. |
| `basin-swift` | Swift | ✅ | Auth, query, REST, storage, functions. Swift Package. |

All SDKs are engine-direct: they talk to `basin-server` over pgwire + REST, no cloud intermediary required.
...
- **Geospatial primary store with full PostGIS** (native `GEOMETRY` column DDL types beyond `POINT`, R-tree over non-`POINT` geometries, `ST_Buffer`/`ST_Union`-style constructive ops) → use real PostGIS or sidecar PG. Basin ships the 2-D geometry codecs + measures + exact predicates, but only `POINT` is a native column type and only `POINT` is R-tree-indexed.
...
- **Vector** — HNSW per-file segment + planner auto-route shipped; native IVF-flat coarse quantiser (k-means `lists` + `ivfflat.probes`) + `vector_avg` aggregate + f32-backed `halfvec(N)` shipped; `sparsevec`/vector-`bit` and `pg_vector` wire-format compat are explicit non-goals (see [ADR 0003](./docs/decisions/0003-native-vector-search.md)).
...
| `CREATE SCHEMA <name>` for user schemas | Phase A shipped: `CREATE SCHEMA` / `DROP SCHEMA` and cross-schema qualified names (`schema.table`) work. `search_path` semantics (unqualified resolution across multiple schemas) and schema-scoped DDL variants are deferred to phases B–E. Reserved schemas (`basin_auth`, `basin_blob`, `basin_cron`, `basin_net`, `basin_realtime`, `pg_catalog`, `information_schema`, `auth`, `storage`, `realtime`) are guarded against user DDL (SQLSTATE 42501). |
...
| `MONEY` | Not supported. |
| `XML` | Not supported. |
| `LINESTRING`, `POLYGON`, geometric types beyond `POINT` | Supported as values (WKB/EWKB/WKT/GeoJSON codecs + `ST_Length`/`ST_Area`/`ST_Centroid`/`ST_Envelope`/`ST_GeometryN`/exact `ST_Contains`/`ST_Intersects`/`ST_Within` etc.), but **not as native column DDL types** — they ride as `BYTEA`/WKB. Only `POINT` has a column type and an R-tree index. |
...
*Last updated: 2026-06-14. This file is hand-maintained; PRs welcome.*
