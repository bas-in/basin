...
2. **Phase 5.16 — Query insights (per-shape stats, scale-regression detection).** Stats system to identify bad query paths commonly used at scale — both for customers (self-service "your slowest queries" view) and for Basin's own product roadmap evidence. OSS side ships plan-shape hashing + per-shape p50/p95/p99 histograms + scale-dependent regression tracking; cloud side ships ingest pipeline + UI + cross-customer anonymized aggregates. Closes the feedback loop on whether our perf commits actually pay off in production, not just in benchmark. Detailed 8-item decomposition in TASK.md Phase 5.16. Bridges OSS ↔ cloud; the cloud-side companion items live in the cloud repo's own roadmap (`docs/basin-cloud-roadmap.md` was removed from this tree in `39fb9f64`).
