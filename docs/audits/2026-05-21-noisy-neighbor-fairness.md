...
# Noisy-Neighbor & Multi-Tenant Fairness Audit

Wedge invariant under test: *one project's behaviour cannot take down another*.
This audit walks every per-tenant resource axis and probes whether (a) a single
hostile/buggy tenant on one server can degrade others, and (b) when N
load-balanced replicas share per-tenant state, hot-tenant pinning or
...
1. **`BASIN_QUERY_COST_LIMIT_ROWS` is opt-in & disabled by default**
   ...
   *unchecked*. A tenant can monopolise a DataFusion executor thread with a
   cartesian self-join for as long as it takes — every other tenant sharing
...
3. **Pgwire connection limiter has no default ceiling**
   ...
   `None` ("unlimited"). OSS / self-hosted tenants get no cap. A
   `BASIN_PGWIRE_RATE_LIMIT_QPS=0` (the default,
   `crates/basin-router/src/rate_limit.rs:112-114`) leaves QPS unlimited too.
   One tenant can open 10k pgwire sockets and DOS the accept loop / file
   descriptors. **P0.**
...
2. **Per-project caps are local — a tenant can multiply their budget by N
   ...
   cross-replica coordinator. A tenant whose REST QPS cap is 100/sec can
...
3. **Catalog Postgres is shared across all replicas.** Every replica connects
   ...
   noisy tenant pushing DDL or `SELECT ... cost_check` adds load directly to
...
| # | Axis | Limiter file:line | Default | Unit | Scales w/ tenants? | Single-instance | Multi-instance | Gap |
|---|------|-------------------|---------|------|--------------------|-----------------|----------------|-----|
...
| 3 | **Statement timeout** | (none) — `grep statement_timeout query_timeout` → no matches | n/a | n/a | n/a | ❌ not present | ❌ not present | A tenant can hold a DataFusion worker indefinitely; full SAT-shape join, recursive CTE, ill-bounded window aggregate |
| 4 | **HTAP project memtable budget** | `crates/basin-hottier/src/budget.rs:27-50, 80-108`; `crates/basin-hottier/src/registry.rs:111-128, 211-261` | hard cap 256 MiB, soft cap 192 MiB, table soft 16 MiB (`budget.rs:27-44`) | per-`ProjectId` `AtomicU64` + `Arc<Semaphore>` (`registry.rs:111-128`) | O(active projects) | ✅ on-by-default; CAS-correct (`registry.rs:220-238`) | ⚠️ per-process — see #4 below | Bursting tenant correctly returns `HardCapReached` (`registry.rs:216-218`); but the **global** scheduler (`budget.rs:228-312`) is also stateless per-process — no cross-replica view |
...
| 10 | **Compactor** | `crates/basin-shard/src/lib.rs:50-51, 73-74`; `crates/basin-shard/src/in_process.rs:684-741` | 30s interval; **single background loop per process** | shared per-process loop (`in_process.rs:718-741`) | n/a — work is FIFO over project map | ⚠️ A tenant with 10k tables makes one tick-pass take longer; quiet tenants' WAL → Parquet drain delayed proportionally | ⚠️ per-replica | No per-project quota; no concurrent compaction lanes |
...
| 11 | **Catalog Postgres connections** | `crates/basin-catalog/src/postgres.rs:57-91` | **one `tokio_postgres::Client` per `PostgresCatalog`**, behind `Mutex<Client>` (`:58`) | shared per process | n/a — one client total | ❌ single mutex serialises every multi-statement transaction; per-query cost-check adds one round-trip per simple-query (`executor.rs:1144-1149`) | ❌ each replica has its own single client; no fleet pool sizing | Docstring `:50-56` admits "For read-heavy workloads we could split into a pool later." A tenant doing 1000 DDLs/min holds the lock 1000× ; everyone else stalls behind |
| 12 | **OTLP export buffer** | `crates/basin-engine/src/query_stats_export.rs:109-122, 144-190` | 15s tick (`:117-122`); emits `tracing::info!` per row | shared per-process exporter task; per-row work `O(distinct (shape, project, table))` | O(active query shapes) | ⚠️ A tenant generating millions of unique shape_hashes (e.g. by injecting literals — see `query_stats.rs` for shape hashing) inflates the registry; per-tick walk grows; one tenant can starve the export thread's CPU budget | ⚠️ per-replica | Each replica exports its own snapshot; collector receives N× the volume |
| 13 | **basin-cron schedules** | `crates/basin-cron/src/runner.rs:104-117, 271-316` | `PER_MINUTE_RATE_LIMIT = 60` runs/project/60s sliding window (`:117`); **single tick-loop per process** (`:170-186`) | per-`ProjectId` `VecDeque<DateTime>` (`:111-112`) | O(active projects) | ✅ rate-limit at 60/min/project; ❌ no cap on number of schedules per project — tenant can `cron.schedule` 10k jobs, each tick walks them all (`:184-186` then `tick_project`) | ❌ per-replica | grep `max_jobs max_schedules max_cron` in `basin-cron/` → no matches. Plus: tick loop calls `engine.exec` for each due job sequentially (`:318-326`) — a slow job blocks the next tenant's job for the same tick |
| 14 | **basin-net outbound HTTP** | `crates/basin-net/src/guards.rs:141-194` | 10 req/s sustained, 30 burst per project (`:163-166`); 10 MiB body cap (`:47`); 30 s timeout (`:48-49`) | per-`ProjectId` token bucket via `governor::keyed`; allowlist per-project in `RwLock<HashMap<ProjectId, HashSet<String>>>` (`:78-82`) | O(active projects × hostnames) | ✅ rate, body, timeout all enforced; ✅ allowlist defaults to DENY-ALL (`crates/basin-net/src/lib.rs:49-53`) | ❌ per-replica | A tenant calling external API can do 10/s × N replicas. Allowlist edit is async + per-process (`:88-103`) — write on replica-A is not seen on replica-B until reconciled |
...
- **OTLP cardinality budget** — none. A tenant generating unbounded
...
|---|----------|-------------|--------------|---------------|
| A | Tenant runs `SELECT * FROM huge_table CROSS JOIN huge_table` | `cost_check` returns `None` for JOIN (`crates/basin-engine/src/cost_check.rs:88-93`); query runs to completion or OOM; no wall-clock timeout | All tenants on same process — DataFusion worker tied up, memory pressure global | (i) Add statement-level wall-clock timeout in `executor.rs` around `df.collect()` and the spawn_blocking branch (`:4541-4565`); (ii) extend `cost_check` to estimate joins via column-cardinality stats |
| B | Tenant opens 10k pgwire sockets | `ConnectionLimiter::unlimited()` admits all (`crates/basin-router/src/connection_limit.rs:228-232`); accept-loop spawns 10k tasks, FDs exhaust | Whole process — pgwire dies; REST may survive | Wire a default ConnectionLimitProvider that reads `BASIN_PGWIRE_MAX_CONNS_PER_PROJECT` env or a project-config table; default 200 |
| C | Tenant does 100k INSERTs/s to one table | Memtable hard cap fires (`crates/basin-hottier/src/registry.rs:216-218`) → `HardCapReached`; subsequent writes return error. ✅ Other tenants OK on memory. **But** the WAL flush task is shared (`crates/basin-wal/src/file_wal.rs:131-141`) — quiet tenants' flushes queue behind | All tenants' WAL durability latency degraded | Bound flush concurrency per project / add per-partition flush parallelism |
| D | Tenant publishes 1M realtime events/sec | `BudgetTracker::try_reserve` returns `BufferFull` cleanly (`crates/basin-realtime/src/budget.rs:98-116`); events routed to webhook retry log per ADR | **Realtime ring OK** — but webhook queue is shared `queue.jsonl` (`crates/basin-webhooks/src/queue.rs:5-12`); failed deliveries fill the queue and the single worker (`crates/basin-webhooks/src/worker.rs:78-89`) ticks through all 1M before getting to anyone else's event | Webhook delivery for **all** projects delayed; disk fills | Per-project queue partition; per-project backlog cap; multi-worker pool |
| E | Tenant creates 10k cron jobs | `register_project` → tick walks all 10k each minute (`crates/basin-cron/src/runner.rs:170-186, 213-260`); per-tick lock held on `last_tick` map (`:189-195`) | All tenants — tick-loop saturated; due jobs for other tenants run late or skipped | Add per-project schedule count cap; parallelise tick across projects |
| F | Tenant pushes catalog-heavy DDL | `Mutex<Client>` (`crates/basin-catalog/src/postgres.rs:58`) serialises every DDL; cost-check `estimate_query_rows` does one PG round-trip per read query (`executor.rs:1142-1153`) | All tenants — every read-path catalog hit waits | Pool the PG client (`deadpool-postgres`); split read-only path onto separate replicas |
| G | Tenant invokes 100k WASM fns/s | Per-project semaphore caps at 16 in-flight (`crates/basin-fn/src/governance.rs:343-353`); excess wait on `acquire_owned` | Project's queue grows unbounded (no admission cap); tokio blocking-pool starvation across all tenants (wasm runs on `spawn_blocking`, `:359`) | Add per-project pending-invocation cap; size blocking-pool with budget per project |
| H | Tenant uploads 10 GiB blobs in a loop | REST `DefaultBodyLimit` = 1 MiB rejects (`crates/basin-rest/src/server.rs:90`); ✅ blocked. **But** if operator raises body cap to support large blobs there's no per-tenant upload-rate or upload-byte cap | Tenant fills object store; bill explodes; no upload-rate throttle exists for blob path | Add per-project `bytes_written_per_sec` token bucket parallel to basin-net's |
| I | Tenant generates query shapes with literals not parameterised | `QueryStatRegistry` (`crates/basin-engine/src/query_stats_export.rs`) shape_hash diverges per literal; registry grows unbounded; OTLP export tick takes longer per cycle | All tenants — exporter thread busy, log volume explodes | Cap distinct shapes per project; LRU eviction in registry |
| J | Tenant flips allowlist 1000×/sec | `AllowList::allow` takes `RwLock::write` per call (`crates/basin-net/src/guards.rs:88-94`); writers block readers (other tenants' HTTP calls stall waiting to look up *their* allowlist) | All outbound HTTP requests stall briefly | Shard the allowlist map per project (DashMap), or copy-on-write |
| K | Tenant opens many WS subscriptions (10k tables × 10k clients) | `ChannelRegistry.channels` DashMap entries persist forever; `prune()` not auto-called (`crates/basin-realtime/src/lib.rs:159-167`); presence map has no per-project cap (`crates/basin-realtime/src/presence.rs`) | Memory growth; iteration cost in subscribe path | Per-project channel count cap; presence per-project cap; auto-prune timer |

---

## 4. Multi-instance hot-tenant pinning analysis

### What pins (one tenant lives on one replica)
...
- **Cross-replica cap evasion**: a tenant whose REST cap is 100 qps can
...
- **Stateless-routed paths (REST / webhooks / wasm via REST / blob)** let a
  tenant multiply any per-project budget by the replica count.
- The wedge invariant "one project can't take down another" is **partially
  satisfied**: noisy-tenant memory and queue bytes are *bounded* per
...
|------|-----|-------------------------|------------|--------|
| **P0-1** | Add statement-level wall-clock timeout in `executor.rs:4541-4565` (both shard and non-shard paths) | All tenants on same process; runaway query holds DataFusion thread + memory | high — easy to trigger accidentally with `JOIN` shapes that bypass `cost_check` | S |
| **P0-2** | Wire a default `ConnectionLimitProvider` (env-var or static config) so OSS deployments get a per-project cap (default 200) instead of `NoConnectionLimit` | All tenants on the process; FD exhaustion, accept-loop saturation | high — trivial DOS | S |
| **P0-3** | Pool the catalog Postgres client (`deadpool-postgres` or `bb8`); split read-only path from txn path | All catalog reads stall behind any DDL on the single `Mutex<Client>` | high — every cost-check is one round-trip; high-DDL tenant blocks all reads | M |
...
| **P1-5** | Multi-instance budget aggregation for billing-impacting caps (memtable bytes, REST QPS): periodic snapshot to catalog; admission compares against fleet total | A tenant on a paid tier can exceed their cap N× by spraying across replicas | medium — observable in metrics; less likely to crash | L |
...
| **P1-8** | Realtime: per-project channel-count cap; presence per-project entry cap; auto-prune timer | Memory leak; per-tenant unbounded | medium | S |
...
| **P2-11** | basin-net allowlist: shard per project (DashMap) or copy-on-write to drop the global writer lock | Allowlist edit stalls other tenants' HTTP calls briefly | low — narrow window | S |
| **P2-12** | Per-project blob upload-byte-rate token bucket (parallel to basin-net's) | Tenant bill explosion; storage IO saturation | medium | M |
...
| **P2-14** | Compactor: per-project concurrency lane(s); cap on small-table compactions per tick | One tenant with 10k small tables monopolises | low — workload-specific | M |
...
2. **Wasm runs on `tokio::task::spawn_blocking`**
   ...
   across all tenants** in the process. With N tenants × 16 permits each
   = potentially 100s of concurrent blocking tasks; the tokio default
   blocking-pool size (512) can be exhausted by a few aggressive tenants,
...
3. **No per-project pending-invocation cap** — a tenant can queue
...
- **1 wedge-violating composite**: a tenant with high DDL + high JOIN
  workload combines (P0-1 + P0-3): catalog mutex held by DDL while a
  bypass-cost-check JOIN burns DataFusion thread — every other tenant's
