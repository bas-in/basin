...
complete per-syntax fragment matrix (975 fragments, auto-generated by
...
> **Fragment counts below are stale.** They describe a 435-fragment corpus;
> the matrix has since grown to **975** fragments and the current figures are
> **863 / 975 = ~88.5% in the default config, ~91.5% non-excluded** (the
> numbers quoted in the README). `docs/sql-support.md` is regenerated by the
> test itself and is the source of truth — read it, not the snapshot here.

Fragment-level coverage at the time of that run: **423 / 435
non-design-excluded fragments = 97.2%**. The remaining 55 fragments were
...
ORM patterns pass. At the fragment level, ~91.5% of non-design-excluded
SQL runs end-to-end across the current 975-fragment matrix. The remaining real gaps are narrow v0.2 items —
...
# Full SQL fragment matrix (975 fragments):
