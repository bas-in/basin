...
ADR's scaffolding) and treats every project as living in that one
...
- **A. Raft-replicated regional WALs.** Each region has its own Raft
  quorum on top of `basin-wal`. Writes for a region-pinned project are
...
- **B. S3 cross-region replication only.** One region is a project's
...
3. **Projects are region-pinned.** Every project has a `home_region`.
...
4. **There is no cross-region 2PC.** A multi-region transaction that
   touches projects pinned to different regions is rejected at plan
...
- **Pure B** under-delivers on regional resilience. Inside a single
  ...
  crash on the WAL host stalls every project pinned to that region
...
- **Pure A** over-engineers the cross-region layer. Per-project Raft
...
- **`home_region: Option<String>` on `TableMetadata`.** Catalogued
  ...
  `set_home_region` is the only mutator. Per-project pinning remains
  out of scope for this ADR — see ADR 0004 for the project-row
  variant; per-table is sufficient to lock the metadata shape, and
  per-project can supersede it additively later.
...
- **S3 CRR is the cross-region durability path.** No bespoke bytes-
  ...
  uses. (BYO-bucket projects configure their own CRR; documented in
  CAPABILITIES.md.)
...
- **Multi-region 2PC.** Transactions that span projects pinned to
...
- **Per-project region pinning at the auth layer.** The scaffolding
  pins per-table; promoting it to per-project (a column on the
  project identity row) is additive and lands when the auth /
  control plane gets a project-metadata row to hang it on.
...
- The storage layout under `projects/{project_id}/...` is region-
  agnostic. S3 CRR replicates it as-is.
- The catalog's project-scoped trait surface
  (`Catalog::load_table(&project, &table)`) does not assume one
...
- Project IDs are ULIDs, not region-namespaced. The scaffolding
  here adds region as a *property* of a project's tables, not as a
  new ID space.
...
- **Pure A — per-project cross-region Raft groups.** Rejected:
...
- **Pure B — no WAL replication, S3 CRR only.** Rejected: leaves
  ...
  stalls every project pinned to that region until manual
...
- **Defer the architectural decision until the trigger fires.**
  ...
  *later*; deferring forces the eventual implementer to do an
  invasive metadata migration on a live multi-project catalog.
