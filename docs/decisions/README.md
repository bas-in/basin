...
| 0021 | YAML frontmatter as the docs metadata contract | Accepted 2026-05-20 | All `docs/**/*.md` files carry YAML frontmatter (title, nav_section, sidebar_position, summary, optional version bounds); MDX and sidecar TOML rejected; enables a downstream docs-site fetch pipeline and CI lint |
...
| 0025 | PG-compat surface phases 5.19–5.30 | Accepted 2026-05-19 | Incremental SQL-compat surface expansion; see body |
...
| 0029 | Distributed-systems primitives Basin deliberately does not carry | Accepted 2026-07-30 | Adopts verifiable release distribution (SHA256SUMS manifest + `scripts/verify.sh` + sigstore provenance). Records, with grep evidence, that Basin has no HLC, no CRDT (reaffirming 0010), no cryptographic node identity (openraft NodeId + operator TLS) and no capability tokens (JWT + RLS is the authorization primitive) — each a design consequence, with the trigger that would change it |
| 0030 | Remove DataFusion; build Basin's own query engine | Accepted 2026-08-12, in progress | DataFusion 53 leaves the dependency tree entirely — no fallback, no trait boundary, no fork. Basin builds its own logical IR, optimizer and physical executor on arrow-rs (kernels stay arrow's), with the logical type system becoming Postgres's rather than Arrow's and `pg_catalog` becoming real relations instead of `TableProvider` shims. Driven by a fidelity ceiling, not a performance one: 9,546-line string rewriter in `pg_operators.rs`, no true SRFs, no DML-as-relation, bypassable RLS. Measured cost ~75k LOC (~115k with tests). Delete is sequenced **last** so the branch stays green; kill criteria recorded |
