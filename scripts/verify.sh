...
#     name as a regex — every "." in "basin-0.1.9-x86_64-…tar.gz" is a wildcard
...
# USAGE
# ─────
#   scripts/verify.sh --tag v0.1.9 basin-0.1.9-x86_64-unknown-linux-gnu.tar.gz
#   scripts/verify.sh --tag v0.1.9 --attest basin-0.1.9-aarch64-apple-darwin.tar.gz
#   scripts/verify.sh --repo vul-os/basin --tag v0.1.9 --out ~/Downloads ASSET
...
trap cleanup EXIT
TMPDIR_V="$(mktemp -d "${TMPDIR:-/tmp}/basin-verify.XXXXXX")"
...
ART   = "basin-9.9.9-x86_64-unknown-linux-gnu.tar.gz"
GOOD  = b"basin-server-release-bytes\n" * 64
...
trap_sums = (line(ART + ".sig", OTHER) +
             line("basin-9X9X9-x86_64-unknown-linux-gnu.tar.gz", OTHER)).encode()
...
run_selftest() {
  ...
  local art="basin-9.9.9-x86_64-unknown-linux-gnu.tar.gz"
  ...
  expect_exit "--attest but no gh installed"  "$E_ATTEST"         yes -- env PATH="$fakebin" "$bash_abs" "$SELF_PATH" --out "$outd" --attest --repo vul-os/basin --base-url "$base/good" "$art"
