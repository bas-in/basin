...
- [x] Confirm license: Apache-2.0 (matches `vul-os/basin`) or MIT — **picked Apache-2.0** (encoded in `LICENSE` full text, `.goreleaser.yaml` `brews.license`, and `README.md`; matches the server-side basin family — `vul-os/basin` engine, `bas-in/basin-cloud` — so the patent grant covers the whole control-plane stack. Client-side SDKs split off to MIT per basin-js decision 2026-05-11.)
...
repo) and `basinctl` (the engine admin CLI in `vul-os/basin`). Single
...
- [ ] Add `Formula/basinctl.rb` — same shape, owned by the basin repo's release pipeline (separate task — see basin/TASKS.md if/when split) — **deferred to basin-engine**: this row is tracked in `vul-os/basin`'s TASKS.md, not here.
...
- [ ] Docker image: `ghcr.io/vul-os/basin:latest` — **scaffolding landed; first GHCR push still gated on user**.
    ...
    - [ ] Actual first push — fires automatically on the v0.1.0 tag push (blocked on the Tier-3 tag), surfaces as `ghcr.io/vul-os/basin:0.1.0` and `:latest`. Verify with `docker pull ghcr.io/vul-os/basin:latest && docker run --rm ghcr.io/vul-os/basin:latest --version`.
...
- [x] **Contract tests against a real cloud** — replay framework landed (`ba242eb`). `tests/contract.rs` + 8 seed JSON fixtures under `tests/contract-fixtures/` exercising projects/orgs/whoami/branches/api-keys/webhooks/error_404/sql_query. Multi-step support (whoami exercises 2 steps: `/auth/v1/user` then `/v1/orgs`). Runner injects `--quiet` to silence the version-check noise that would otherwise trip the drift detector. **Record mode** (generate fresh fixtures from a live basin-cloud) intentionally left as future work — gated on live cloud + format-stability decision.
