//! activity — STUB. Pending Rust port from the Go `cmd_activity.go`.
...
pub fn cmd_activity(_g: &GlobalFlags, _args: &[String]) -> CliResult<()> {
    Err(msg("basin activity: not yet ported to the Rust CLI"))
}
