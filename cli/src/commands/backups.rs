//! backups — STUB. Pending Rust port from the Go `cmd_backups.go`.
...
pub fn cmd_backups(_g: &GlobalFlags, _args: &[String]) -> CliResult<()> {
    Err(msg("basin backups: not yet ported to the Rust CLI"))
}
