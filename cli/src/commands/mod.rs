...
pub fn all() -> Vec<Entry> {
    vec![
        ...
        entry!(
            "logout",
            "Forget the locally-stored token.",
            logout::cmd_logout
        ),
        entry!(
            "whoami",
            "Print the active user + accessible orgs.",
            whoami::cmd_whoami
        ),
        entry!(
            "orgs",
            "List / get / create / update / delete orgs + branding.",
            orgs::cmd_orgs
        ),
        entry!(
            "members",
            "List / invite / remove / role-change org members.",
            members::cmd_members
        ),
        entry!(
            "invitations",
            "List / resend / revoke pending org invitations.",
        ...
        entry!(
            "link",
            "Bind ./basin/ to a remote project (writes project_ref).",
            link::cmd_link
        ),
        entry!(
            "status",
            "Show the linked project + migration drift summary.",
            status::cmd_status
        ),
        entry!(
            "unlink",
        ...
        entry!(
            "db",
            "Push / pull / diff / reset / url / dump / lint the project database.",
            db::cmd_db
        ),
        entry!(
            "tables",
            "List / describe / create / alter / drop tables + CSV import/export.",
            tables::cmd_tables
        ),
        entry!(
            "rows",
        ...
        entry!(
            "rls",
            "Enable / disable RLS + manage row-level security policies.",
            rls::cmd_rls
        ),
        entry!(
            "migrations",
            "List / apply / new / get / diff / rollback DDL migrations.",
            migrations::cmd_migrations
        ),
        entry!(
            "branches",
            "List / create / get / merge / delete / events on project branches.",
        ...
        entry!(
            "backups",
            "Manage backup policy / snapshots / restore jobs.",
            backups::cmd_backups
        ),
        entry!(
            "logs",
        ...
        entry!(
            "realtime",
            "Subscribe to table event streams over SSE (T26.1).",
            realtime::cmd_realtime
        ),
        entry!(
            "api-keys",
            "List / create / rotate / revoke project-scoped API keys.",
            api_keys::cmd_api_keys
        ),
        entry!(
            "pgwire",
            "Show / reveal / rotate pgwire credentials and engine JWT keys.",
            pgwire::cmd_pgwire
        ),
        entry!(
            "domains",
            "Add / verify / cert / list / remove custom project domains.",
            domains::cmd_domains
        ),
        entry!(
            "webhooks",
            "Manage project webhooks: create / patch / test / redeliver / deliveries.",
            webhooks::cmd_webhooks
        ),
        entry!(
            "alerts",
            "Manage alert rules + view fired events.",
            alerts::cmd_alerts
        ),
        entry!(
            "audit",
            "List org audit events (--org=<slug> required).",
            audit::cmd_audit
        ),
        entry!(
            "activity",
        ...
        entry!(
            "metrics",
            "Show project or org metrics (--range / --metric).",
            metrics::cmd_metrics
        ),
        entry!(
            "erd",
            "Export project ERD (svg / dot / json).",
            erd::cmd_erd
        ),
        entry!(
            "queries",
            "Save / list / get / fork / delete / history SQL queries.",
            queries::cmd_queries
        ),
        entry!(
            "engine-pin",
            "Get / put / delete / events on the project's engine version pin.",
            engine_pin::cmd_engine_pin
        ),
        entry!(
            "extensions",
            "List the project's installed engine extensions.",
            extensions::cmd_extensions
        ),
        entry!(
            "oauth-providers",
            "List / get / set / delete project OAuth providers.",
            oauth_providers::cmd_oauth_providers
        ),
        entry!(
            "email",
            "Manage project email templates + view allowance.",
            email::cmd_email
        ),
        entry!(
            "byo",
            "Configure bring-your-own bucket / kms / engine.",
            byo::cmd_byo
        ),
        entry!(
            "saml",
            "Get / put / test / enable / disable org SAML SSO.",
            saml::cmd_saml
        ),
        ...
        entry!(
            "docs",
            "Open docs.basin.run for a given subcommand.",
            docs::cmd_docs
        ),
        entry!(
            "secrets",
        ...
        entry!(
            "tokens",
            "Manage org-scoped personal access tokens.",
            tokens::cmd_tokens
        ),
        entry!(
            "config",
            "Read or write CLI config (telemetry, default org).",
        ...
        entry!(
            "version",
            "Print binary version + build metadata.",
            version::cmd_version
        ),
        entry!(
            "help",
