//! orgs — STUB. Pending Rust port from the Go `cmd_orgs.go`.
...
pub fn cmd_orgs(_g: &GlobalFlags, _args: &[String]) -> CliResult<()> {
    Err(msg("basin orgs: not yet ported to the Rust CLI"))
}
