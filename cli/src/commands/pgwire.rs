//! pgwire — STUB. Pending Rust port from the Go `cmd_pgwire.go`.
...
pub fn cmd_pgwire(_g: &GlobalFlags, _args: &[String]) -> CliResult<()> {
    Err(msg("basin pgwire: not yet ported to the Rust CLI"))
}
