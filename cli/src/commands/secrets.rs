//! secrets — STUB. Pending Rust port from the Go `cmd_secrets.go`.
...
pub fn cmd_secrets(_g: &GlobalFlags, _args: &[String]) -> CliResult<()> {
    Err(msg("basin secrets: not yet ported to the Rust CLI"))
}
