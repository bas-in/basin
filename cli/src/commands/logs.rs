//! logs — STUB. Pending Rust port from the Go `cmd_logs.go`.
...
pub fn cmd_logs(_g: &GlobalFlags, _args: &[String]) -> CliResult<()> {
    Err(msg("basin logs: not yet ported to the Rust CLI"))
}
