//! tables — STUB. Pending Rust port from the Go `cmd_tables.go`.
...
pub fn cmd_tables(_g: &GlobalFlags, _args: &[String]) -> CliResult<()> {
    Err(msg("basin tables: not yet ported to the Rust CLI"))
}
