//! init — STUB. Pending Rust port from the Go `cmd_init.go`.
...
pub fn cmd_init(_g: &GlobalFlags, _args: &[String]) -> CliResult<()> {
    Err(msg("basin init: not yet ported to the Rust CLI"))
}
