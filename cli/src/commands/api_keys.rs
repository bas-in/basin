//! api-keys — STUB. Pending Rust port from the Go `cmd_api_keys.go`.
...
pub fn cmd_api_keys(_g: &GlobalFlags, _args: &[String]) -> CliResult<()> {
    Err(msg("basin api-keys: not yet ported to the Rust CLI"))
}
