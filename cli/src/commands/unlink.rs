//! unlink — STUB. Pending Rust port from the Go `cmd_unlink.go`.
...
pub fn cmd_unlink(_g: &GlobalFlags, _args: &[String]) -> CliResult<()> {
    Err(msg("basin unlink: not yet ported to the Rust CLI"))
}
