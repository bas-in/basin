//! email — STUB. Pending Rust port from the Go `cmd_email.go`.
...
pub fn cmd_email(_g: &GlobalFlags, _args: &[String]) -> CliResult<()> {
    Err(msg("basin email: not yet ported to the Rust CLI"))
}
