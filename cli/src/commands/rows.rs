//! rows — STUB. Pending Rust port from the Go `cmd_rows.go`.
...
pub fn cmd_rows(_g: &GlobalFlags, _args: &[String]) -> CliResult<()> {
    Err(msg("basin rows: not yet ported to the Rust CLI"))
}
