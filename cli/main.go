...
func commands() []commandEntry {
	return []commandEntry{
		...
		{"db", "Push / pull / diff / reset / url / dump / lint the project database.", cmdDB},
		{"tables", "List / describe / create / alter / drop tables + CSV import/export.", cmdTables},
		{"rows", "Insert / update / delete rows in a project table.", cmdRows},
		{"rls", "Enable / disable RLS + manage row-level security policies.", cmdRLS},
		...
		{"domains", "Add / verify / cert / list / remove custom project domains.", cmdDomains},
		{"webhooks", "Manage project webhooks: create / patch / test / redeliver / deliveries.", cmdWebhooks},
		{"alerts", "Manage alert rules + view fired events.", cmdAlerts},
		{"audit", "List org audit events (--org=<slug> required).", cmdAudit},
		{"activity", "List project activity events.", cmdActivity},
		{"metrics", "Show project or org metrics (--range / --metric).", cmdMetrics},
		{"erd", "Export project ERD (svg / dot / json).", cmdERD},
		{"queries", "Save / list / get / fork / delete / history SQL queries.", cmdQueries},
		{"engine-pin", "Get / put / delete / events on the project's engine version pin.", cmdEnginePin},
		{"extensions", "List the project's installed engine extensions.", cmdExtensions},
		{"oauth-providers", "List / get / set / delete project OAuth providers.", cmdOAuthProviders},
		{"email", "Manage project email templates + view allowance.", cmdEmail},
		{"byo", "Configure bring-your-own bucket / kms / engine.", cmdBYO},
		{"saml", "Get / put / test / enable / disable org SAML SSO.", cmdSAML},
		{"scim", "SCIM config + provisioning tokens.", cmdSCIM},
		{"oauth-apps", "Manage org OAuth applications (clients).", cmdOAuthApps},
		{"docs", "Open docs.basin.run for a given subcommand.", cmdDocs},
