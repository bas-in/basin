//! Tenant -> shard endpoint mapping (consistent-ish hashing + whale pins).
//!
...
//!
//! Some tenants are 100× the size of the median. Letting the
...
/// Maps `TenantId -> shard endpoint`. Cheap to clone.
#[derive(Clone, Debug)]
pub struct ShardMap {
    endpoints: Vec<String>,
    pins: HashMap<TenantId, usize>,
}
...
impl ShardMap {
    ...
    pub fn with_pins(
        ...
        for (tenant, idx) in &pins {
            if *idx >= endpoints.len() {
                return Err(format!(
                    "pin for tenant {tenant} points at endpoint index {idx} \
    ...
    /// this `ShardMap`. If the tenant is pinned, the pinned endpoint
    ...
    /// 0-based index of the shard that owns `tenant`. Pinned tenants
    ...
    pub fn shard_index(&self, tenant: &TenantId) -> usize {
        if let Some(idx) = self.pins.get(tenant) {
            return *idx;
        }
    ...
    /// True if this tenant has an explicit pin (vs falling through to
    /// the consistent-hash path). Useful for telemetry / log correlation.
    pub fn is_pinned(&self, tenant: &TenantId) -> bool {
        self.pins.contains_key(tenant)
    }

    /// Number of distinct pinned tenants. Surfaced for startup logs so
...
/// Parse the `BASIN_TENANT_PINS` env-var format into a `(TenantId, usize)`
...
pub fn parse_pins_env(s: &str) -> Result<HashMap<TenantId, usize>, String> {
    let s = s.trim();
    if s.is_empty() {
...
mod tests {
    ...
    #[test]
    fn pinned_tenant_lands_on_pinned_endpoint() {
    ...
    #[test]
    fn pinned_tenant_overrides_what_hash_would_pick() {
        // Construct a tenant, see which shard the hash would assign,
    ...
    #[test]
    fn unpinned_tenants_still_use_hash_path() {
        let whale = TenantId::new();
        ...
        // The unpinned tenant's index must match what the bare hash gives
        // — pinning one tenant must not perturb anyone else.
    ...
    fn parse_pins_env_rejects_bad_ulid() {
        ...
        assert!(err.contains("invalid tenant ulid"), "got {err}");
    }
