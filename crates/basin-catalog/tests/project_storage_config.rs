//! Integration tests for the per-project storage config catalog API.
//!
...
use basin_catalog::{Catalog, InMemoryCatalog, PostgresCatalog, ProjectStorageConfig};
use basin_common::ProjectId;
use ulid::Ulid;

fn sample_config(key: &str) -> ProjectStorageConfig {
...
async fn set_then_get_round_trips_in_memory() {
    ...
    cat.set_project_storage_config(&t, cfg.clone())
        .await
        .unwrap();
    let got = cat.get_project_storage_config(&t).await.unwrap();
...
async fn set_overwrites_existing_in_memory() {
    let cat = InMemoryCatalog::new();
    let t = ProjectId::new();
    cat.set_project_storage_config(&t, sample_config("first"))
        .await
        .unwrap();
    cat.set_project_storage_config(&t, sample_config("second"))
        .await
        .unwrap();
    let got = cat.get_project_storage_config(&t).await.unwrap().unwrap();
...
async fn cross_project_isolation_in_memory() {
    ...
    let a = ProjectId::new();
    let b = ProjectId::new();
    cat.set_project_storage_config(&a, sample_config("for-a"))
        .await
        .unwrap();
    let got_a = cat.get_project_storage_config(&a).await.unwrap().unwrap();
    let got_b = cat.get_project_storage_config(&b).await.unwrap();
...
async fn drop_namespace_clears_config_in_memory() {
    let cat = InMemoryCatalog::new();
    let t = ProjectId::new();
    cat.set_project_storage_config(&t, sample_config("k"))
        .await
        .unwrap();
...
#[tokio::test]
async fn cross_project_isolation_postgres() {
    let Some((cat, _guard)) = try_connect().await else {
        return;
    };
    let a = ProjectId::new();
...
async fn survives_simulated_restart_postgres() {
    ...
    let cat2 = PostgresCatalog::connect_with_schema(PG_URL, &schema)
        .await
        .expect("reconnect");
    let got = cat2.get_project_storage_config(&t).await.unwrap();
    assert_eq!(got, Some(cfg));
}
