...
pub trait Catalog: Send + Sync {
    ...
    // multi-region WrongRegion write gate reads (`basin-engine` region.rs), so
    // there is no separate `*_project_home_region` accessor: the placement
    // admin route read-modify-writes `ProjectMetadata`.
