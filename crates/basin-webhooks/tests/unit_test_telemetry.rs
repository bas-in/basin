//! Per-tenant telemetry tests for the webhook fanout machinery.
//!
...
//! per-tenant counters. Each test asserts a single observable property
//! of the counters API.
...
async fn metrics_increment_on_successful_delivery() {
    ...
    for seq in 1..=5 {
        let e = evt(tenant, "orders", ChangeOp::Insert, seq);
...
async fn metrics_increment_on_retry_then_success() {
    ...
    let e = evt(tenant, "orders", ChangeOp::Insert, 1);
    stack.sink.publish(&e).await.unwrap();
...
/// Two tenants each fire matching events. A's counters do not pick up
/// B's events and vice-versa.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn metrics_per_tenant_isolated() {
    let server = spawn_server(200).await;
    let net = make_client();
    let a = TenantId::new();
    let b = TenantId::new();
...
/// driving the dequeue counter manually via `for_tenant().record_dequeue()`
...
async fn pending_queue_depth_tracks_correctly() {
    ...
    // pop 7 entries directly off the queue and bump the per-tenant
    ...
    for _ in 0..7 {
        ...
        stack.counters.for_tenant(&entry.tenant).record_dequeue();
    }
    let snap = stack.counters.snapshot(&tenant);
...
/// reading 0). We stay inside basin-net's per-tenant burst cap (30
