...
use datafusion::common::{Result, ScalarValue, tree_node::{Transformed, TreeNode}};
...
fn rewrite_nullif_expr(expr: Expr) -> Result<Transformed<Expr>> {
    match &expr {
        Expr::ScalarFunction(sf) if sf.name().eq_ignore_ascii_case("nullif") => {
            ...
            // Use a typed-Null scalar so type-coercion sees the right branch
            // type.  ScalarValue::Null is the untyped null sentinel; DataFusion
            // promotes it to the type of the ELSE branch during analysis.
            let then_expr = Box::new(Expr::Literal(ScalarValue::Null, None));
