...
async fn email_token_single_use(store: &Arc<dyn AuthStore>) {
    ...
    assert_eq!(n2, 0, "second consume must update 0 rows (already consumed)");
...
async fn tenant_credential_uniqueness(store: &Arc<dyn AuthStore>) {
    ...
    assert!(!second, "second insert with same pgwire_user must return false (conflict)");
}
