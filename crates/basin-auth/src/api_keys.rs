//! Per-project long-lived API keys.
//!
...
//! ## Key format
//!
//! Keys are issued in the format `basin_{project_id}_{random_urlsafe_base64}`,
//! where `project_id` is the 26-char ULID. This makes the key self-routing:
//! callers can extract the project from the key without a DB round-trip.
//!
...
/// Extract the 26-character project ULID from an API key of the form
/// `basin_{26-char-ulid}_{url-safe-base64}`. The embedded project makes keys
/// self-routing: callers can identify the project without a DB round-trip before
...
pub fn parse_project_from_api_key(raw_key: &str) -> Option<&str> {
...
pub(crate) async fn validate(inner: &Inner, raw: &str) -> Result<(ProjectId, UserId)> {
    let raw = raw.trim();
    if raw.is_empty() {
    ...
    for row in rows {
        ...
        if password::verify(raw, &row.key_bcrypt)? {
            ...
            let project: ProjectId = row
                .project_id
                .parse()
                .map_err(|e| BasinError::internal(format!("api_key project parse: {e}")))?;
            return Ok((project, row.user_id));
        }
    }
    Err(BasinError::InvalidIdent("invalid api key".into()))
}

pub(crate) async fn revoke(inner: &Inner, key_id: i64, project: &ProjectId) -> Result<()> {
    inner.store.revoke_api_key(project, key_id).await
}
...
pub(crate) async fn list(
    ...
    inner.store.list_api_keys(project, user).await
}
...
mod tests {
    ...
    #[test]
    fn key_format_contains_project() {
        let project = ProjectId::new();
        let secret = generate_secret(&project);
        assert!(secret.starts_with("basin_"));
        let project_str = project.to_string();
        assert!(secret.contains(&project_str));
    }

    #[test]
    fn parse_project_from_api_key_round_trips() {
        let project = ProjectId::new();
        let secret = generate_secret(&project);
        let parsed = parse_project_from_api_key(&secret).expect("should parse");
        assert_eq!(parsed, project.to_string());
    }

    #[test]
    fn parse_project_from_api_key_rejects_wrong_format() {
        assert!(parse_project_from_api_key("notakey").is_none());
        assert!(parse_project_from_api_key("basin_short").is_none());
        assert!(parse_project_from_api_key("").is_none());
    }

    // --- parse_project_from_api_key: comprehensive cases ---------------------

    #[test]
    fn parse_project_valid_full_key() {
        // basin_{26-char ulid}_{base64 random} → Some(ulid)
        let project = ProjectId::new();
        let project_str = project.to_string();
        let secret = format!("basin_{project_str}_somebase64stuff");
        let parsed = parse_project_from_api_key(&secret).expect("must parse");
        assert_eq!(parsed, project_str.as_str());
    }

    #[test]
    fn parse_project_wrong_prefix_returns_none() {
        let project = ProjectId::new();
        let project_str = project.to_string();
        // Prefix is "notbasin_" instead of "basin_"
        let bad = format!("notbasin_{project_str}_extra");
        assert!(parse_project_from_api_key(&bad).is_none());
        // No prefix at all
        assert!(parse_project_from_api_key(&format!("{project_str}_extra")).is_none());
    }

    #[test]
    fn parse_project_missing_random_suffix_returns_none() {
        let project = ProjectId::new();
        ...
        let no_payload = format!("basin_{}_", project);
        assert_eq!(no_payload.len(), 6 + 26 + 1);
        // rest = "{project}_" has len 27, which is not > 27 → None
        assert!(parse_project_from_api_key(&no_payload).is_none());
    }

    #[test]
    fn parse_project_short_after_basin_prefix_returns_none() {
        assert!(parse_project_from_api_key("basin_shortkey").is_none());
        assert!(parse_project_from_api_key("basin_").is_none());
    }

    #[test]
    fn parse_project_empty_string_returns_none() {
        assert!(parse_project_from_api_key("").is_none());
    }
    ...
    fn parse_project_self_routing_invariant() {
        ...
        // project it was generated for, regardless of the random payload.
        for _ in 0..20 {
            let project = ProjectId::new();
            let secret = generate_secret(&project);
            let parsed =
                parse_project_from_api_key(&secret).expect("generated secret must always parse");
            assert_eq!(
                parsed,
    ...
    fn generate_secret_random_suffix_is_url_safe() {
        ...
        let project = ProjectId::new();
        let secret = generate_secret(&project);
        // Skip "basin_{project}_" prefix (6 + 26 + 1 = 33 chars)
