...
fn generate_markdown(rows: &[MatrixRow], timestamp: &str, total: usize) -> String {
    ...
    // The frontmatter block is emitted HERE, by the generator, not hand-added
    // to docs/sql-support.md — the next run of this test overwrites that file,
    // so a hand-edit would be silently reverted and the `docs frontmatter` CI
    // gate would go red again with no obvious cause. Keep the fields in sync
    // with docs/frontmatter-spec.md (nav_section must be one of its enum
    // values; summary must be <= 200 chars).
    let mut out = format!(
        r#"---
title: "Basin SQL support — auto-generated matrix"
nav_section: sql
sidebar_position: 90
summary: "Generated support matrix for every SQL fragment the test corpus exercises, across all three parser/planner configurations. Regenerated by the sql_support_matrix test."
---
