...
async fn last_basic_correctness() {
    ...
    exec(&sess, "INSERT INTO fl_last VALUES
         (10, '2024-01-01 10:00:00+00'),
         (20, '2024-01-01 09:00:00+00'),
         (30, '2024-01-01 11:00:00+00')").await;
    ...
    assert_eq!(result, 30, "last should return value at max ts (30), got={result}");
...
async fn first_last_null_ts_ignored() {
    ...
    exec(&sess, "INSERT INTO fl_null VALUES
         (99, NULL),
         (10, '2024-01-01 10:00:00+00'),
         (20, '2024-01-01 08:00:00+00')").await;
...
async fn first_last_group_by() {
    ...
    exec(&sess, "CREATE TABLE fl_grp (grp TEXT, v INT, ts TIMESTAMPTZ)").await;
    exec(&sess, "INSERT INTO fl_grp VALUES
         ('a', 1, '2024-01-01 10:00:00+00'),
         ('a', 2, '2024-01-01 09:00:00+00'),
         ('b', 3, '2024-01-01 10:00:00+00'),
         ('b', 4, '2024-01-01 11:00:00+00')").await;
...
async fn first_last_empty_group_null() {
    ...
    assert_eq!(n, 1, "empty aggregate should produce 1 row with NULL, got={n}");
...
async fn drop_chunks_removes_old_preserves_new() {
    ...
    exec(&sess, "SELECT create_hypertable('dc_tbl', 'ts', chunk_time_interval => INTERVAL '1 day')")
        .await;

    // Old rows (before 2024-01-01): should be dropped.
    exec(&sess, "INSERT INTO dc_tbl VALUES
         (1, '2023-11-01 00:00:00+00'),
         (2, '2023-12-01 12:00:00+00'),
         (3, '2023-12-31 23:59:59+00')").await;

    // New rows (2024 and after): should survive.
    exec(&sess, "INSERT INTO dc_tbl VALUES
         (4, '2024-01-01 00:00:00+00'),
         (5, '2024-06-15 12:00:00+00'),
         (6, '2024-12-31 23:59:59+00')").await;

    let before = single_i64(&sess, "SELECT COUNT(*) FROM dc_tbl").await;
    assert_eq!(before, 6, "expected 6 rows before drop_chunks, got={before}");

    // Drop chunks older than 2024-01-01 (absolute cutoff).
    exec(&sess, "SELECT drop_chunks('dc_tbl', older_than => TIMESTAMP '2024-01-01 00:00:00')").await;
...
async fn drop_chunks_interval_cutoff() {
    ...
    exec(&sess, "SELECT create_hypertable('dc_iv', 'ts', chunk_time_interval => INTERVAL '1 day')")
        .await;

    // Old rows (several years ago) → will fall outside any reasonable retention.
    exec(&sess, "INSERT INTO dc_iv VALUES
         (1, '2020-01-01 00:00:00+00'),
         (2, '2020-06-01 12:00:00+00')").await;
    ...
    exec(&sess, "INSERT INTO dc_iv VALUES (3, '2099-12-31 00:00:00+00')").await;

    let before = single_i64(&sess, "SELECT COUNT(*) FROM dc_iv").await;
    assert_eq!(before, 3);

    // Drop chunks older than 1 year (relative cutoff).
    exec(&sess, "SELECT drop_chunks('dc_iv', older_than => INTERVAL '1 year')").await;
    ...
    assert_eq!(after, 1, "after interval drop_chunks: only 2099 row should remain, got={after}");
...
async fn drop_chunks_no_hot_tier_ghost() {
    ...
    exec(&sess, "CREATE TABLE dc_hot (id BIGINT, v TEXT, ts TIMESTAMPTZ)").await;
    exec(&sess, "SELECT create_hypertable('dc_hot', 'ts', chunk_time_interval => INTERVAL '1 day')")
        .await;

    exec(&sess, "INSERT INTO dc_hot VALUES
         (1, 'old_1', '2022-03-15 00:00:00+00'),
         (2, 'old_2', '2022-03-16 00:00:00+00'),
         (3, 'new_1', '2099-01-01 00:00:00+00')").await;

    // Drop everything older than 2023-01-01.
    exec(&sess, "SELECT drop_chunks('dc_hot', older_than => TIMESTAMP '2023-01-01')").await;
    ...
    assert_eq!(rows, 0, "no 'old' rows should survive drop_chunks, found={rows}");

    let surviving = single_i64(&sess, "SELECT id FROM dc_hot").await;
    assert_eq!(surviving, 3, "only row id=3 ('new_1') should survive, got={surviving}");
...
async fn retention_policy_physically_deletes() {
    ...
    exec(&sess, "SELECT create_hypertable('rp_tbl', 'ts', chunk_time_interval => INTERVAL '1 day')")
        .await;
    // Retention: 1 year (365 days).
    exec(&sess, "SELECT add_retention_policy('rp_tbl', INTERVAL '365 days')").await;

    // Ancient rows (>1 year ago) → should be deleted.
    exec(&sess, "INSERT INTO rp_tbl VALUES
         (1, '2020-01-01 00:00:00+00'),
         (2, '2020-06-01 00:00:00+00')").await;
    // Future row → must survive.
    exec(&sess, "INSERT INTO rp_tbl VALUES (3, '2099-12-31 00:00:00+00')").await;
...
async fn time_bucket_2arg_epoch_aligned_pin() {
    ...
    exec(&sess, "INSERT INTO tb2arg VALUES
         ('2024-01-01 00:45:00+00'),
         ('2024-01-01 01:15:00+00'),
         ('2024-01-01 01:35:00+00')").await;

    // 2-arg epoch-aligned: 00:45→00:00 bucket, 01:15+01:35→01:00 bucket → 2 buckets.
    let n = row_count(&sess,
        "SELECT time_bucket('1 hour', ts) FROM tb2arg GROUP BY 1").await;
    assert_eq!(n, 2, "epoch-aligned: 3 rows should yield 2 hour buckets, got={n}");
...
async fn time_bucket_3arg_origin_aligned_pin() {
    ...
    exec(&sess, "INSERT INTO tb3arg VALUES
         ('2024-01-01 00:45:00+00'),
         ('2024-01-01 01:15:00+00'),
         ('2024-01-01 01:35:00+00')").await;
...
async fn time_bucket_origin_shifts_boundary() {
    ...
    exec(&sess, "INSERT INTO tb_orig VALUES
         ('2024-01-01 00:55:00+00'),
         ('2024-01-01 01:00:00+00'),
         ('2024-01-01 01:05:00+00')").await;

    // Epoch-aligned: 00:55→00:00, 01:00+01:05→01:00 → 2 buckets.
    let n_epoch = row_count(&sess,
        "SELECT time_bucket('1 hour', ts) FROM tb_orig GROUP BY 1").await;
    ...
    let n_origin = row_count(&sess,
        "SELECT time_bucket('1 hour', ts, TIMESTAMP '2024-01-01 00:45:00') \
         FROM tb_orig GROUP BY 1").await;
    assert_eq!(n_origin, 1,
...
async fn hypertables_view_queryable() {
    ...
    assert!(r.is_ok(), "hypertables view must be queryable: {:?}", r.unwrap_err());
...
async fn hypertables_view_retention_secs() {
    ...
    let secs_before = single_i64(&sess,
        "SELECT retention_secs FROM timescaledb_information.hypertables \
         WHERE hypertable_name = 'ht_ret_v'").await;
    assert_eq!(secs_before, -1,
        "no retention policy → retention_secs must be -1, got={secs_before}");

    // After setting a 30-day retention policy.
    exec(&sess, "SELECT add_retention_policy('ht_ret_v', INTERVAL '30 days')").await;
    let secs_after = single_i64(&sess,
        "SELECT retention_secs FROM timescaledb_information.hypertables \
         WHERE hypertable_name = 'ht_ret_v'").await;
    let expected_secs = 30 * 86400i64; // 2592000
    assert_eq!(secs_after, expected_secs,
...
async fn hypertables_view_num_chunks() {
    ...
    exec(&sess, "INSERT INTO ht_nc VALUES
         (1, '2024-05-01 10:00:00+00'),
         (2, '2024-05-02 10:00:00+00'),
         (3, '2024-05-03 10:00:00+00')").await;

    let num_chunks = single_i64(&sess,
        "SELECT num_chunks FROM timescaledb_information.hypertables \
         WHERE hypertable_name = 'ht_nc'").await;
    assert!(num_chunks >= 3,
