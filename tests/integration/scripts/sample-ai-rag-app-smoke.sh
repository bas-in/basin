...
# A missing app directory is a FAILURE, not a skip. examples/ai-rag-app/ is
...
if [[ ! -d "${APP_DIR}" ]]; then
  fail "examples/ai-rag-app/ not found at ${APP_DIR}. It is tracked in git, so this
...
log "npm test"
TEST_LOG="$(mktemp "${TMPDIR:-/tmp}/ai-rag-app-smoke-test.XXXXXX")"
