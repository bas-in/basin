...
- **Latest run:** 2026-05-07 16:36:10 UTC
...
| **Large-dataset point query (real S3)** | **PASS** | `7.87 ms` | `< 8000` |
...
| **S3 credentials and round-trip** | **PASS** | `42.00 ms` | `< 2000` |
...
- **S3 credentials and round-trip** — Real S3-compatible service handles PUT, GET, LIST, DELETE with byte-accurate body round-trip. Measured `PUT+GET+LIST+DELETE total` = `42.00 ms`.
...
**peak speed-up vs C=1:** `6.53×` (bar `≥ 3.5`)

| concurrent readers | Total QPS (qps) | Per-task QPS (qps) | Median latency (us) |
| --- | --- | --- | --- |
| 1 | 284.33 | 284.33 | 3425.00 |
| 4 | 1190.33 | 297.58 | 888.00 |
| 16 | 1819.67 | 113.73 | 3598.00 |
| 64 | 1858.00 | 29.03 | 19911.00 |
...
**point query p50 growth (1M / 10K):** `1.59×` (bar `< 5`)

| row count | Disk (MiB) | Bytes / row (B) | Point query p50 (ms) | Full scan p50 (ms) |
| --- | --- | --- | --- | --- |
| 10,000 | 0.09 | 9.02 | 2.22 | 1.67 |
| 100,000 | 0.85 | 8.96 | 2.43 | 11.62 |
| 1,000,000 | 8.54 | 8.95 | 3.51 | 108.35 |
...
| 100 | 0.48 | 0.07 | 48 |
| 1,000 | 0.64 | 0.56 | 640 |
| 5,000 | 0.61 | 2.53 | 3,056 |
| 10,000 | 0.86 | 5.91 | 8,560 |

### Noisy-neighbor degradation (real S3) — **FAIL**

_On real S3, a heavy noisy tenant doesn't crater a quiet tenant's p99 latency._

**p99 ratio (under_load / baseline):** `74.45×` (bar `< 5`)

| scenario | p50 latency (ms) | p99 latency (ms) |
| --- | --- | --- |
| baseline | 1.16 | 1.37 |
| under_load | 0.75 | 102.17 |
| ratio | 0.65 | 74.45 |
...
**speedup p50 (a→d):** `1.41×` (bar `≥ 5`)

| Stack layer | p50 latency (ms) | p99 latency (ms) |
| --- | --- | --- |
| a_baseline | 6.65 | 7.99 |
| b_disk | 2.72 | 12.39 |
| c_disk_page | 3.10 | 13.24 |
| d_full | 4.70 | 15.67 |
...
**basin delete_ms at largest scale:** `1492.42 ms` (bar `< ∞`)
...
|---|---|---|---|---|
| Backup wall time | `0.00 s` | `0.18 s` | **Basin** | `pg / basin = 107x` |
...
| Metric | Basin | Postgres | Winner | Ratio |
|---|---|---|---|---|
| Tenant deletion (1 tenant, 100K rows in 100 files; basin on S3) | `11.13 ms` | `7.72 ms` | **Postgres** | `pg / basin = 0.69x` |
| ADD COLUMN on 100K rows | `0.09 ms` | `8.09 ms` | **Basin** | `pg / basin = 93.08x` |
...
| Point query p50 | `0.97 ms` | `2.69 ms` | **Basin** | `pg / basin = 2.77x` |
| Insert 100K rows | `275.63 ms` | `329.09 ms` | **Basin** | `—` |
...
| Metric | Basin | Postgres | Winner | Ratio |
|---|---|---|---|---|
| Connection accept latency p50 | `0.39 ms` | `1.41 ms` | **Basin** | `pg / basin = 3.62x` |
| Connections held under 1000-conn flood | `1000.00 conns` | `97.00 conns` | **Basin** | `basin / pg = 10.31x` |
| Refused conns under 1000-conn flood | `0.00 conns` | `903.00 conns` | **Basin** | `—` |
| RSS per held-open connection | `109.28 KiB` | `7571.52 KiB` | **Basin** | `pg / basin = 69.29x` |
