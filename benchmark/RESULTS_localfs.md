...
- **Latest run:** 2026-05-09 12:44:00 UTC
- **Viability:** 35 / 37 passing
- **Scaling:** 8 / 9 passing
...
| **Analytical path speedup vs OLTP engine** | **FAIL** | `1.38×` | `≥ 1.5` |
| **Analytical routing speedup through basin-engine** | **FAIL** | `1.45×` | `≥ 1.5` |
...
| **Cluster-by physical sort overhead on insert** | **PASS** | `0.81×` | `< 2` |
| **CLUSTER BY SQL surface (Phase 5.7 B2)** | **PASS** | `5 variants` | `= 5` |
| **Cold-start latency (LocalFS)** | **PASS** | `141.75 ms` | `< 2000` |
...
| **Shard-acked INSERT path** | **PASS** | `18440.7 inserts/sec` | `≥ 50` |
...
| **Native vector search (HNSW + brute-force agree)** | **PASS** | `2.13×` | `≥ 1.3` |
...
- **Analytical path speedup vs OLTP engine** — On a 1M-row aggregate (count + avg + sum-of-lengths) with GROUP BY across 10 Parquet files, the DuckDB-backed analytical engine beats the DataFusion-on-listing-table OLTP engine by at least 1.5x on the same dataset. Measured `analytical_speedup (engine_ms / analytical_ms)` = `1.38×`.
- **Analytical routing speedup through basin-engine** — Same SQL through the same engine entry point: with the analytical engine attached, GROUP BY queries route to DuckDB and finish at least 1.5x faster than the OLTP-only baseline on a 1M-row dataset spread across 10 Parquet files. Measured `analytical_routing_speedup (oltp_ms / routed_ms)` = `1.45×`.
...
- **Cold-start latency (LocalFS)** — From process spawn to first `SELECT 1` over pgwire is < 2 s on LocalFS storage. Measured `cold_start_ms` = `141.75 ms`.
...
- **Shard-acked INSERT path** — Single-row INSERTs route through the WAL + shard owner and acks fast enough that point INSERT throughput is no longer Parquet-bound. Measured `inserts_per_sec` = `18440.7 inserts/sec`.
...
- **Native vector search (HNSW + brute-force agree)** — HNSW top-10 overlaps brute-force top-10 by >=7, HNSW serves the same query at <100ms. Measured `HNSW speed-up vs brute-force` = `2.13×`.
...
**peak speed-up vs C=1:** `6.95×` (bar `≥ 3.5`)

| concurrent readers | Total QPS (qps) | Per-task QPS (qps) | Median latency (us) |
| --- | --- | --- | --- |
| 1 | 416.33 | 416.33 | 2401.00 |
| 4 | 1645.00 | 411.25 | 123.00 |
| 16 | 2893.33 | 180.83 | 555.00 |
| 64 | 2827.67 | 44.18 | 11422.00 |

### Single-tenant data size scale-up — **PASS**

_Storage is linear in row count; point-query latency stays bounded._

**point query p50 growth (10M / 100k):** `1.04×` (bar `< 5`)

| row count | Disk (MiB) | Bytes / row (B) | Point query p50 (ms) | Full scan p50 (ms) |
| --- | --- | --- | --- | --- |
| 100,000 | 0.79 | 8.25 | 3.82 | 5.69 |
| 1,000,000 | 7.86 | 8.24 | 3.12 | 56.04 |
| 10,000,000 | 78.59 | 8.24 | 3.97 | 575.08 |
...
| 100 | 0.48 | 0.14 | 48 |
| 1,000 | 0.90 | 1.23 | 896 |
| 5,000 | 0.72 | 4.62 | 3,616 |
| 10,000 | 0.94 | 8.76 | 9,424 |
...
**speedup p50 (a→d):** `377.78×` (bar `≥ 100`)

| Stack layer | p50 latency (ms) | p99 latency (ms) |
| --- | --- | --- |
| a_baseline | 584.18 | 589.73 |
| b_disk | 2.03 | 603.53 |
...
**quiet_p50_at_max / quiet_p50_at_1:** `0.71×` (bar `< 5`)
...
**basin delete_ms at largest scale:** `179.81 ms` (bar `< ∞`)
...
**Basin/Postgres ratio at 5000 files (realistic schema):** `19.6529 ratio` (bar `< 1`)
...
|---|---|---|---|---|
| Backup wall time | `0.00 s` | `0.22 s` | **Basin** | `pg / basin = 2320x` |
...
| Metric | Basin | Postgres | Winner | Ratio |
|---|---|---|---|---|
| Tenant deletion (1 tenant, 100K rows) | `5.45 ms` | `6.00 ms` | **Basin** | `pg / basin = 1.10x` |
| ADD COLUMN on 100K rows | `0.21 ms` | `6.50 ms` | **Basin** | `pg / basin = 30.42x` |
...
| Point query p50 | `0.15 ms` | `14.21 ms` | **Basin** | `pg / basin = 96.92x` |
| Insert 1M rows | `2351.60 ms` | `2494.09 ms` | **Basin** | `—` |
...
| Metric | Basin | Postgres | Winner | Ratio |
|---|---|---|---|---|
| Connection accept latency p50 | `0.46 ms` | `1.47 ms` | **Basin** | `pg / basin = 3.17x` |
...
| RSS per held-open connection | `122.24 KiB` | `7624.80 KiB` | **Basin** | `pg / basin = 62.38x` |
