...
- **Latest run:** 2026-05-09 13:27:26 UTC
...
**peak speed-up vs C=1:** `6.79×` (bar `≥ 3.5`)

| concurrent readers | Total QPS (qps) | Per-task QPS (qps) | Median latency (us) |
| --- | --- | --- | --- |
| 1 | 389.67 | 389.67 | 2595.00 |
| 4 | 1477.33 | 369.33 | 139.00 |
...
**point query p50 growth (10M / 100k):** `1.21×` (bar `< 5`)

| row count | Disk (MiB) | Bytes / row (B) | Point query p50 (ms) | Full scan p50 (ms) |
| --- | --- | --- | --- | --- |
| 100,000 | 0.79 | 8.25 | 3.71 | 5.60 |
...
| 100 | 2.56 | 0.13 | 256 |
| 1,000 | 0.93 | 0.92 | 928 |
| 5,000 | 0.83 | 4.55 | 4,144 |
| 10,000 | 0.99 | 9.11 | 9,888 |
...
| Point query p50 | `0.18 ms` | `15.06 ms` | **Basin** | `pg / basin = 82.64x` |
| Insert 1M rows | `2877.69 ms` | `3063.77 ms` | **Basin** | `—` |
...
| Metric | Basin | Postgres | Winner | Ratio |
|---|---|---|---|---|
| Connection accept latency p50 | `0.44 ms` | `1.46 ms` | **Basin** | `pg / basin = 3.33x` |
...
| RSS per held-open connection | `120.96 KiB` | `7657.92 KiB` | **Basin** | `pg / basin = 63.31x` |
